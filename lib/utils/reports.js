...
// This mess is temporary. When this becomes a class per scenario,
// we won't have to mess with stuff like this as much
const MISC_ARTIFACTS_DIR = `_alkiln-misc_artifacts`;
console.log(1, session_vars.get_artifacts_path_name());
let debug_path, verbose_path;
...
if ( !session_vars.get_artifacts_path_name() ) {
  ...
  debug_path = `${ MISC_ARTIFACTS_DIR }/unit_tests_logs_debug-${ files.readable_date() }.txt`;
  verbose_path = `${ MISC_ARTIFACTS_DIR }/unit_tests_logs_verbose-${ files.readable_date() }.txt`;
  console.log(2)

  try {
    console.log(3)
    fs.mkdirSync( MISC_ARTIFACTS_DIR, { recursive: true });
    console.log(4)
  } catch ( error ) {
    console.log(5)
...
console.log(6, `debug_path:`, debug_path, `verbose_path:`, verbose_path );

let log = new Log({ debug_path, verbose_path });
...
reports.addReportHeading = function (scope, { scenario }) {
  ...
  */
  log.debug({ type: `info`, code: `ALK0175`, pre: `reports.addReportHeading(): ${scenario.pickle.name} \n` });  // console
...
reports.addToReport = function( scope, { type, code, value }) {
  ...
  * */
  // Debugging logs
  let debug_line = getDebugLine({ type, code, value });  // Already contains type and codes at the moment
  // log.debug({ pre: `reports.addToReport():\ntype: ${ type }, value: ${ value }\n` });  // console
  // log.debug_log({ pre: `Report:\ntype: ${ type }, value: ${ value }` });  // debug log file
