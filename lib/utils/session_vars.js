...
session_vars.just_test_interview_list = function () { return process.env.JUST_TEST_INTERVIEW_LIST || null; };
...
session_vars.save_artifacts_path_name = function ( path_name ) {
  ...
  // If file already exists, get the current JSON. Otherwise make new JSON
  let json = null;
  try {
    json = JSON.parse( fs.readFileSync( runtime_config_path ) );
  } catch ( error ) {
    json = {};
  }

  // Change the JSON to what we need now and re-write the file from scratch with the new value
