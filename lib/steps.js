...
After(async function(scenario) {
  ...
  if (changeable_test_status !== `PASSED`) {
    ...
    // if (changeable_test_status === `FAILED`) {
      reports.addToReport(scope, { type: `outcome failure info`, code: `ALK0096`, value: `**-- Scenario ${ changeable_test_status.charAt(0).toUpperCase() + string.slice(1).toLowerCase() } --**` });
    // }
...
console.log(25)
  ...
  if ( need_to_force_failed_status ) {
    ...
    log.info({ type: `internal error`, code: `ALK0208`, pre: `${errors.join( errors, '\n')}` });
  }
