...
const yaml_to_js = require('js-yaml');
...
Before(async (scenario) => {
  ...
  scope.toDownload = '';
  scope.custom_filename_to_save = '';
  // To use for comparison
  scope.filename_to_use_for_comparison = '';
  // Device interaction
  scope.device = 'pc';
...
Given(/I start the interview at "([^"]+)"(?: in lang "([^"]+)")?/, {timeout: -1}, async (file_name, language) => {  // √
  ...
  while ( num_attempts < max_load_attempts ) {
    ...
    let loaded = new Promise(async ( resolve, reject ) => {
      ...
      let page_timeout = setTimeout(function() {
        if ( num_attempts === max_load_attempts ) { reject( `${ timeout_message }` ); }
  ...
  // Prepare folder for a scenario's downloaded files
  ...
  let custom_download_behavior_timeout = new Promise(async ( resolve, reject ) => {
    ...
    let page_timeout = setTimeout(function() {reject( `Took too long to set download behavior. Internal error.` ); }, scope.timeout);
    // self-invoke an anonymous async function so we don't have to
    // abstract the details of resolving and rejecting.
    (async function() {
      ...
      try {
        await scope.page._client.send('Page.setDownloadBehavior', {
          behavior: 'allow',
          downloadPath: path.resolve( docs_dir ),  // Save folder in root of this project
        });
        resolve();
  ...
  scope.page.on('response', async response => {
    // Don't remove event listener since we might download multiple files.
    // Listeners don't seem to die with a specific page.

    // Note: I believe we can expect that only one file will be getting downloaded
    ...
    if ( response._headers['content-disposition'] && (response._headers['content-disposition']).includes(`attachment;filename=${scope.toDownload}`) ) {
      ...
      const original_filename = path.basename( response.request().url() );
      console.log( `filename: ${ original_filename }` );
      console.log( `concat to get dir: ${ docs_dir }/${ original_filename }` );
      console.log( `path.resolve: `, path.resolve( docs_dir, original_filename ));
      let basename = scope.custom_filename_to_save;
      if ( !basename ) {  // If scope filename is empty string or something
        // Maybe add page id here for only non-customized names
        // Maybe add page id after for adding to all filenames
        basename = original_filename;
      }
      let new_filename = basename;
      let max_tries = 30  // Capping while loops is always a good idea
      let count = 1;  // Create unique filename if necessary
      ...
      await fs.watchFile( docs_dir, function (curr, prev) {
        ...
        if (parseInt(curr.size) === parseInt(response._headers['content-length'])) {
          // If the new filename already exists, it will be overwritten:
          // https://nodejs.org/api/fs.html#fsrenameoldpath-newpath-callback
          // Has this first file already been overwritten? We need to check if
          // downloading this way produces a unique name in the folder already
          // or if it's overwritten another file.
          while ( fs.existsSync( `${ docs_dir }/${ new_filename }` ) && count <= max_tries ) {
            new_filename = basename + count;
            count++;
          }
          ...
          if ( count > max_tries ) {
            await scope.addToReport( scope, {
              type: `warning`,
              value: `You can't save over ${ max_tries } files with the `
              + `same base name, "${ basename }", in one scenario. This new `
              + `file was saved with the name "${ original_filename }". If `
              + `you need more files, contact the ALKiln developers.`
            });
          } else {
            scope.filename_to_use_for_comparison = new_filename;

            fs.renameSync(
              `${ docs_dir }/${ original_filename }`,
              `${ docs_dir }/${ new_filename }`
            });
          }
          ...
          // this.close();  // What does this close? Don't want to close page.
          return true;
        }

        // If a relevant file was downloaded, reset filename so
        // it doesn't get mixed up with the next filename.
        scope.custom_filename_to_save = '';
      });  // ends fs.watchFile
...
When(/I download "([^"]+)" it matches "([^"]+)"/, async (filename_on_page, to_compare_to) => {
  /* Compares one file to another. Currently only YAML files.
  *
  * @param filename_on_page {str} - part of the url of a file that appears
  *    on the page and can be downloaded.
  * @param to_compare_to {str} - name of file in the "comparison_files"
  *    folder or the "Sources" folder that the new file should match. */

  let supported_extensions = [ `.yml` ];
  let contains_supported_extention = false;
  for ( let extension of supported_extensions ) {
    if ( to_compare_to.includes( extension )) {
      contains_supported_extention = true;
    }
  }

  if ( !contains_supported_extention ) {
      await scope.addToReport( scope, {
        type: 'warning',
        value: `Cannot compare "${ filename_on_page }" to "${ to_compare_to }". `
        + `ALKiln can only support comparing files with these extensions: `
        + `${ supported_extensions.join(', ') }.`
      });
  }

  // TODO: Somehow ensure file extensions of both files match

  // Save with the same name as the original file so the developer/user
  // can match the two later if they want to examine them manually.
  await scope.setDownloadFilename( scope, { filename: to_compare_to });
  await scope.download( scope, { filename: filename_on_page });

  // Load both yml files and turn them into JSON objects
  let original_yml = await scope.getDeveloperFile( scope, { to_compare_to });
  let new_yml = await scope.getDeveloperFile( scope, { filename_on_page });

  let original_json = yaml_to_js( original_yml );
  let new_json = yaml_to_js( new_yml );
...
When(/I download "([^"]+)"$/, async (filename) => {
  ...
  *
  * WARNING: Cannot download the same file twice in a single scenario without
  *    a custom name. */
  await scope.download(scope, { filename });
});
