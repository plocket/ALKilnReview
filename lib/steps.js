...
Then(/I get the var(?:iable)? val(?:ue)?s(?: on the page)?/, async () => {
  /** Uses  https://docassemble.org/docs/functions.html#js_get_interview_variables
  *    to get the variable values on the page of the interview. */

  let vars_obj_eventually = await scope.page.evaluate(async function ( elem ) {
    let obj_promise = get_interview_variables(function ( data ) {
      if ( data.success ){
        let vars_obj = data.variables;
        return data;
      }
    });
    return obj_promise;
  ...
  await vars_obj_eventually;
  console.log( vars_obj_eventually.variables );
