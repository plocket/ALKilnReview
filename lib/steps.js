    ...
    let custom_timeout = new Promise(async function( resolve, reject ) {
      ...
      (async function() {
        try {
          ...
          let path = `${ scope.paths.random_screenshots }/random-${curr_page_num}-${ short_id }${ Date.now() }.jpg`;
          ...
          let forward_buttons_exist = false;
          // If there were any buttons, pick a random one to try to press
          if ( buttons_exist ) {
            forward_buttons_exist = await scope.set_random_input_for.buttons( scope, { fields: buttons });
          }

          if ( !buttons_exist && !forward_buttons_exist ) {
