...
Given(/I start the interview at "([^"]+)"(?: in lang "([^"]+)")?/, async (file_name, language) => {  // √
  ...
  let { id, id_matches } = await scope.examinePageID( scope, 'none to match' );
...
When(/^(?:the user gets)?(?:I get)? to ?(?:the)?(?: ?question)?(?: id)? "([^"]+)" with this data:?/, {timeout: -1}, async (target_id, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  while ( !id_matches ) {
    ...
    // let reject_on_timeout = function( desired_timeout, err_msg ) {
    ...
    //     }, desired_timeout );
    //   });
    //   promise.cancel = function () { clearTimeout( timeout_id ); };
    //   return promise;
    // };

    // try {

    //   let some_promise = delay();
    //   await Promise.race([
    ...
    //     //     reject( Error( timeout_message ));  // rationale: code communicates it's an error
    //     //   }, scope.timeout );
    //     // }),

    //     (async function() {
    //       let { id, id_matches } = await scope.examinePageID( scope );
    ...
    let custom_timeout = new Promise(async function( resolve, reject ) {
      // Make sure the timeout rejects or tests will hang at the end no matter what
      let timeout_id = setTimeout(function() {
        let timeout_message = `The target id was "${ target_id }", but it took too long` +
        ` to try to reach it (over ${ scope.timeout/1000/60 } min). The test got stuck on "${ id }".`
        reject( Error( timeout_message ));  // rationale: code communicates it's an error
      ...
      try {
        ...
        clearTimeout( timeout_id );  // prevent temporary hang at end of tests
      }
