...
Before(async (scenario) => {
  ...
  if (!scope.browser) {
    if ( session_vars.get_origin() !== `local` ) {
      ...
      // scope.browser = await scope.driver.launch({ headless: false, devtools: true });
    }
  }
  ...
  let scenario_state = scope.scenarios.get( scope.scenario_id )
  scenario_state.interviews = {};
  scenario_state.api_keys = [];
  // TODO: add a Step to set a max id custom value
  // TODO: implement setting current page
  scenario_state.pages = { alk_max_same_id_count: 30, current: null };
  scenario_state.navigation = {
    expected: false,
    actual: false,
    override_the_expected: false,
    override_val: null,
    failure_type: null
  };
...
Given(
  ...
  */
  ...
  reports.addToReport( scope, { type: `info`, code: `ALK0204`, value: `Trying to sign into ${ session_vars.get_da_server_url() }.` });
    ...
    let custom_timeout = new Promise(function( resolve, reject ) {
      ...
      (async function() {
        try {
          ...
          let error = await scope.setFields(scope, { var_data: ensured_var_data, id, tap_continue_if_needed: true });
...
Then(/I ?(?:should)? see the phrase "([^"]+)"/i, async ( phrase ) => {  // √
  ...
  let msg = `The text "${ phrase }" SHOULD be on this page, but it's NOT. The actual text:\n${ JSON.stringify( bodyText ) }`;
...
Then(/I should not see the phrase "([^"]+)"/i, async ( phrase ) => {  // √
  ...
  * "ignores hidden elements" */
  ...
  let msg = `The text "${ phrase }" should NOT be on this page, but it IS here. The actual text:\n${ JSON.stringify( bodyText ) }`;
...
// ctrl+g (path cmd+p)
// Then I tap to continue, but can’t/cannot/don’t/do not keep going/continue
Then(/tapping the continue button fails/i, { timeout: -1 }, async () => {
  /** A way to tap 'continue' and then fail to continue so that an author can
   * then check for invalid answer messages. */
  let scenario = scope.get_scenario( scope );
  scope.override_expected_navigation( scope, { override_val: false });
  // `page` timeout will deal with custom timeout
  await scope.setFields( scope, { var_data: [], tap_continue_if_needed: true });
});

// Then(/I will (?:be told)?(?:see)? an answer is invalid/, async () => {  // √
...
When(/I tap to continue$/i, { timeout: -1 }, async () => {
  ...
  // No data will cause an automatic continue when `tap_continue_if_needed` is true
...
When(/I tap the "([^"]+)" element(?: and go to (?:a new|the next) page)?$/i, { timeout: -1 }, async ( elemSelector ) => {
  // `page` timeout will deal with custom timeout
  //TODO:
  scope.override_expected_navigation();
...
When(/I tap the "([^"]+)" element and stay(?: on the same page)?$/i, { timeout: -1 }, async ( selector ) => {
  /**
   * Ex:
   * - I tap the "#copy_button" element and stay on the same page
   * - I tap the "#copy_button" element and stay */
  // `page` timeout will deal with custom timeout
  //TODO:
  scope.override_expected_navigation();
...
// AfterStep(async ( cucumber_scenario ) => {
//   /** Do checks and resets that need to happen before (after?) each Step. */

//   // Check for system error? Before each Step starts instead?



//   // TODO: How will this mesh with "took to long to get to"
//   // TODO: "took to long to get to" message of over 30 secs is
//   // confusing? What was it expecting to do in those 30 seconds?
//   // Finish the test from start to finish?
//   // TODO: Change the `I (don't|can't|don’t|can’t|cannot|do not) continue` Step

//   // let scenario = scope.get_scenario( scope );
//   // let nav = scenario.navigation;

//   // // --- Check that navigation happened correctly and act accordingly ---

//   // if ( scope.page ) {

//   //   // TODO: Should we check for trigger var change. It would fail
//   //   // faster than 15 page ids. Would indicate failed navigation,
//   //   // not necessarily infinite loop. Could be invalid input. Check for such?
//   //   // That might be done where we check for navigation.

//   //   // TODO: names: validate, constrain
//   //   // TODO: Can't be in `AfterStep`. This can happen in the middle of a StoryTable.
...
After(async function(scenario) {
  ...
  if (changeable_test_status !== `PASSED`) {
    ...
    // if (changeable_test_status === `FAILED`) {
      // reports.addToReport(scope, { type: `outcome failure info`, code: `ALK0096`, value: `**-- Scenario ${ changeable_test_status.charAt(0).toUpperCase() + changeable_test_status.slice(1).toLowerCase() } --**` });
      reports.addToReport(scope, { type: `outcome info`, code: `ALK0096`, value: `**-- Scenario ${ changeable_test_status.charAt(0).toUpperCase() + changeable_test_status.slice(1).toLowerCase() } --**` });
    // }
...
console.log(11)
  ...
  if ( scope.expected_in_report && scope.expected_in_report.length > 0) {
    ...
    if ( !report_includes_all_expected_strings ) {
      console.log(11.3); changeable_test_status = `FAILED`; }
  ...
  if ( changeable_test_status === `PASSED`
    ...
    if ( changeable_test_status === scope.expected_status ) {
      ...
      let msg = `Test ${ changeable_test_status }. Expected status: `
        + `${scope.expected_status}. See the report for details.`;
      let code = `ALK0099`;
  ...
  let force_failure_msg = `ALKiln assessed that the test failed. cucumberjs said the scenario ${scenario.result.status}. See the report for details.`;
  ...
  // If cucumber thinks the test passed, we still need cucumber to throw an error
  // If it's SKIPPED or something, it can carry on with that.
...
console.log(18)
  ...
  // If there is a page open, then sign out
  if ( scope.page ) {
  console.log(2)
    // Catch a possible reload error.
    try {

      console.log(`\n\n============\n`, await scope.page.url());
  ...
  console.log(7)
          let code = `ALK0203`;
          let msg = `ALKiln tried to sign out to prepare for the next test, but it took too long and then another error occurred. Maybe the tests disconnected from the server`;
          // reports.addToReport( scope, { type: `internal error`, code: code, value: msg });
          log.debug({ type: `internal error`, code, pre: msg, data: error });
          signout_error_msg = `🤕 ${ code } internal ERROR: ${ msg }: ${ error }`;
        }
      } else {

  console.log(8)
        // Warn about any non-timeout error
        let warning_msg = `ALKiln tried to sign out to prepare for the next test, but ran into a problem. Maybe the tests disconnected from the server`
        let code = `ALK0098`;
        console.log(error);
        console.log(scope.browser);
        reports.addToReport( scope, { type: `warning`, code: code, value: `${ warning_msg }: ${ error }` });
        signout_error_msg = `🔎 ${ code } WARNING: ${ warning_msg }: ${ error }`;
...
console.log(25)
  ...
  if ( need_to_force_failed_status ) {
    ...
    throw new Error( `🤕 ${ force_failure_code } unreachable ERROR: This error should be unreachable. ${ force_failure_msg }` );
  ...
  // Otherwise fail or pass the normal way for users

  // TODO: Find the race condition.
