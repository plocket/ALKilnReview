...
When(/^the user gets to ?(?:the)?(?: ?question)?(?: id)? "([^"]+)" with this data:?/, {timeout: -1}, async (target_id, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  // | var_name choice_name value |
  // | set var_name choice choice_name to value |
  let rows = raw_var_data.raw();
  let var_data = rows.map(( row ) => {
    // let split = row.split(' ');
    // return { var: split[0], choice: split[1], value: split[2] };

    let split = row.match(`\set (.*) ?(?:choice (.*))? to (.*)\ig`);
    return { var: split[0], choice: split[1], value: split[2] };
  });
