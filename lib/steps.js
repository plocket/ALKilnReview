...
Given(/I (?:sign|log) ?(?:in)?(?:on)?(?:to the server)? with(?: the email)? "([^"]+)" and(?: the password)? "([^"]+)"(?: SECRETs)?/i, {timeout: -1}, async ( email, password ) => {
  ...
  */
  // Exiting the custom timeout happens in scope.tapToNavigate() if needed
  // Couldn't test the timeout for tapping the sign in button because there's not
...
When('I tap to continue', { timeout: -1 }, async () => {
  ...
  // Exiting the custom timeout happens in scope.tapToNavigate() where we can
  // check if the server is reloading
  await scope.setFields( scope, { var_data: [], ensure_navigation: true });
});

When(/I tap the "([^"]+)" tab/i, async ( tabId ) => {
  await scope.tapTab( scope, tabId );
});

When(/I tap the "([^"]+)" element$/i, { timeout: -1 }, async ( elemSelector ) => {
  /** TODO: Reconsider this step as it currently is:
  * Tapping arbitrary elements is a bit nuts since we have no good way to
  * detect navigation. And we need to know if navigation is expected so we
  * can do navigation stuff, like detecting server reloads and undisabling
  * screenpics for secret vars. 
  */
  // Exiting the custom timeout happens in scope.tapToNavigate() if appropriate
    ...
    let custom_timeout = new Promise(async function( resolve, reject ) {
      ...
      (async function() {
        try {
          ...
          console.log('buttons exist?', buttons)
          if ( buttons_exist ) {
...
When(/I download "([^"]+)"$/, { timeout: -1 }, async ( filename ) => {
  ...
  */
  // TODO: This should be able to detect server reload.
  return wrapPromiseWithTimeout(
...
After(async function(scenario) {
  ...
  if (scenario.result.status !== `PASSED`) {
    ...
    if ( scope.page ) {
      ...
      // Take screenshot if it's not disabled. Unless we're debugging, in which case
      // still take a screenshot.
      if ( !scope.disable_error_screenshot || session_vars.get_debug() ) {
