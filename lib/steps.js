...
BeforeAll(async () => {
  ...
  // File has already been created in run_cucumber.js
  scope.paths.verbose_log = `${ scope.paths.artifacts }/${ log.verbose_log_file }`;
...
Then(/of two interviews, one is deleted __internal__/i, async () => {
...
After(async function(scenario) {
  ...
  if ( scenario.result.message ) {
    log.debug({ type: `info`, code: `ALK0091`, pre: `cucumber error: ${ scenario.result.message }` });
  }
  ...
  // TODO: Test behavior when no user is logged in
  ...
  try {
    ...
    if ( scenario.test_interview_deletion ) {
      ...
      // // Mocks are useless!
      // // await quietly_try_to_delete_loaded_interviews( scope );  // See below
  ...
  log.debug({ type: `cucumber`, code: `ALK0100`, pre: `Scenario AfterStep() message`, data: scenario.result.message });
