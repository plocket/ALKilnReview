    ...
    try {
      ...
      if ( error.name === `TimeoutError` ) {
        ...
        try {
          ...
          let code = `ALK0203`;
          let msg = `ALKiln ran into an error after timing out when trying to sign out in preparation for the next test: ${ error }`;
          // reports.addToReport( scope, { type: `internal error`, code: code, value: msg });
          signout_error_msg = `🤕 ${ code } internal ERROR: ${ msg }`;
        }
      } else {
        ...
        signout_error_msg = `🤕 ${ code } ERROR: ${ other_err_msg }: ${ error }`;
