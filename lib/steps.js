...
// Can't be instantiated until artifacts path is saved later
let log = null;
...
Before(async (scenario) => {
  ...
  console.log('');  // Add a new line and also show this in the GitHub console as tests run
  ...
  fs.mkdirSync( scope.paths.scenario, { recursive: true });
...
After(async function(scenario) {
  ...
  if ( scenario.result.message ) {
    log.debug({ type: `info`, code: `ALK0091`, pre: `cucumber error Scenario result: ${ scenario.result.message }` }); //, verbose: true });
  }
