...
BeforeAll(async () => {
  ...
  // TODO: remove writing to debug_log file
...
When(/^(?:the user gets)?(?:I get)? to ?(?:the)?(?: ?question)?(?: id)? "([^"]+)" with this data:?/, {timeout: -1}, async (target_id, raw_var_data) => {
  ...
  * Iterate in here to keep timeout reasonable */
  ...
  let { id, id_matches } = await scope.examinePageID( scope, target_id );
  // Until we reach the target page, hit an error, or time out
  while ( !id_matches ) {
    ...
    let custom_timeout = new Promise(function( resolve, reject ) {
      ...
      let on_page_timeout = async function() {
        ...
        //   console.log( `b` );
        //   // Call this function recursively once the server is responding again
        //   // some_func( scope, { resolve, reject, supported_table })
        // // If it wasn't a server reload issue, throw the error as usual
        // } else {
        //   console.log( `c` );

        // For now, just timeout. scope.tapElement will take care of giving extra info
        // about server reloading problems.
        // In future, call this function recursively once the server is responding
          // TODO: Add error to report
          await scope.addToReport( scope, { type: `error`, value: timeout_message });
          // Rejection currently doesn't cause an error
      ...
      (async function() {
        try {
          ...
          // Some errors happen in there to be closer to the relevant code. To otherwise
          // get this in here, we'd have to try/catch, which seems a waste.
    ...
    // Prep for next loop
    ({ id, id_matches } = await scope.examinePageID( scope, target_id ));
