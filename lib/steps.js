...
Given(/I start the interview at "([^"]+)"(?: in lang "([^"]+)")?/, {timeout: -1}, async (file_name, language) => {  // √
  // Handling our own timeouts to deal with initial scenario load.
  ...
  while ( num_attempts <= max_load_attempts ) {
    console.log(`num_attempts`, num_attempts);
    ...
    let custom_load_timeout = new Promise(async ( resolve, reject ) => {
      ...
      let page_timeout = setTimeout(function() {
        if ( num_attempts === max_load_attempts ) { reject( `${timeout_message} Internal timeout.` ); }
          ...
          console.log( "resolving", num_attempts );
        resolve(); }  // Give it another shot
      }, scope.timeout);
      ...
      (async function() {
        try {
          ...
          if ( num_attempts === max_load_attempts ) {
            ...
            reject( err );
          } else { resolve(); }  // Give it another shot
        } finally {
          clearTimeout( page_timeout );  // prevent temporary hang at end of tests
        }
      })();
    });  // ends custom_load_timeout
    await custom_load_timeout;
  }  // ends while attempting to load page


  // await scope.load( scope, file_name );

  // Why does this timeout setting need to happen in here since we have a step for it?
  // Won't really timeout so doesn't need a custom timeout
...
When(/^(?:the user gets)?(?:I get)? to ?(?:the)?(?: ?question)?(?: id)? "([^"]+)" with this data:?/, {timeout: -1}, async (target_id, raw_var_data) => {
  /* NO CUCUMBER TIMEOUT. Must handle timeout ourselves since we can't caluclate it beforehand based
