...
function parse_arguments(just_args) {
  let argv = require('minimist')(just_args);
  ...
  for (let source_folder of argv["source"]) {
    provided_config.paths.push(source_folder + "/*.feature");
    session_vars.set_sources_folder(source_folder);
  }

  tag_args = argv["_"];
  if (tag_args.length > 0) {
    ...
    if (tag_args.every((val) => val.startsWith("@"))) {
      provided_config.tags = tag_args.join(' or ');
    }
    // If we get a list of seemingly no tags, assume they meant to use tags
    else if (tag_args.every((val) => !val.startsWith("@"))) {
      provided_config.tags = tag_args.map((val) => "@" + val).join(' or ');
    }
    // If it's a mix of tags and no tags, it's probably a boolean expression!
    else {
      provided_config.tags = tag_args.join(' ');
    }
  }
