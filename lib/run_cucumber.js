...
(async function main() {
  ...
  *   manipluate the output. */
  ...
  // Start log file as early as possible
  start_artifacts();
  ...
  fs.readFile(summary_file_name, 'utf8', (err, text) => {
    if (err) {
      log.error({ type: `run`, code: `ALK0056`, pre: `Failed to read cucumber report`, data: err });
      return;
    }
    ...
    try {
      fs.appendFileSync( `${ artifacts_path }/${ log.verbose_log_file }`, `\n\n${text}`);
    } catch ( error ) {
      log.info({ type: `run`, code: `ALK9999`, pre: `Something probably went wrong earlier. Look above for more details, but here is more information just in case: ALKiln was unable to write to ${ artifacts_path }/${ log.verbose_log_file }.`, data: error });
    }
    try {
