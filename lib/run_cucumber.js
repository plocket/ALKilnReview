...
function parse_arguments(just_args) {
  ...
  let to_remove_idxs = [];
  for (let idx = 0; idx < just_args.length; idx++) {
    let arg = just_args[idx];
    if (arg.startsWith("--")) {
      // A flag! make sure the flag is expected
      if (arg === "--sources") {
        if (idx + 1 >= just_args.length) {
          log.error({pre: `the --sources flag needs a glob path, but nothing else is provided!`, data: just_args});
          process.exitCode = 2;
          return null;
        }
        ...
        log.error({pre: `No flag named ${arg}`, data: just_args});
        process.exitCode = 2;
        return null;
      }
    }
  ...
  to_remove_idxs.reverse();
  for (let remove_idx of to_remove_idxs) {
    just_args.splice(remove_idx, 2);
  }
...
(async function main() {
  let provided_config = parse_arguments(process.argv.slice(2));
  if (!provided_config) {
    return;
  }
