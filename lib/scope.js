...
module.exports = {
  ...
  detectDownloadComplete: async function detectDownloadComplete(scope, endTime) {
    ...
    *    the global permitted timeout */
    ...
    // Stop/resolve if too much time has passed. Won't exit otherwise?
    let expirationTime = scope.timeout;  // ms
    ...
    if ( scope.downloadComplete ) {
      ...
      await scope.page.waitFor( 98 );
      return await scope.detectDownloadComplete(scope, endTime);
    }
  },  // Ends scope.detectDownloadComplete()

  download: async function (scope, { filename }) {
  ...
  setDownloadFilename: async function ( scope, { filename }) {
    /** Set global name for file that needs to be downloaded.
    *    Ensure safte filename. */
    scope.custom_filename_to_save = safe_filename( filename );
  },

  getDeveloperFile: async function ( scope, { filename }) {
    /** Try different directories to get the developer's file.
    *    This is mostly needed for concurrent dev of ALKiln itself. */
    // May want to pull this up a level, but is it worth another param?
    let package_name = env_vars.REPO_URL.match(/docassemble-[^\/]*/)[0];
    console.log( `package_name: ${ package_name }` );

    let filepaths = [
      path.resolve( `./comparison_files`, filename ),
      path.resolve( `./docassemble/${ package_name }/data/sources/`, filename ),
    ];
  ...
  getDownloadedFile: async function ( scope, { filename }) {
    /** Try different directories to get the developer's file.
    *    This is mostly needed for concurrent dev of ALKiln itself. */
    // May want to pull this up a level, but is it worth another param?

    let filepath = path.resolve( `./comparison_files`, filename )
