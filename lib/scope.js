...
module.exports = {
  ...
  set_random_input_for: {
    ...
    buttons: async function (scope, { fields }) {
      /** Get a random button other than back-button then return true. If
      *    none of those were found, return false. */
      let back_btn = null;

      // If the page only has a 'back' button, random tests have reached
      // an end. How to stop now?
      filtered_fields = fields.filter((field_to_filter) => {
        if (field_to_filter.selector.includes("daquestionbackbutton")) {
          return false;
        }
        return true;
      });
      ...
      if ( filtered_fields.length === 0 ) {
        // Just skip it? Shouldn't happen but has before, though don't recall details.
        // What else can the random tests do to continue, though?
        return false;  // no non-back-button buttons found
      }

      let field = faker.helpers.arrayElement( filtered_fields );
      let handle = await scope.get_handle_from_field( scope, { field: field });
      // Send it to be tapped
      await scope.funnel_the_answer( scope, { handle, field, answer: `doesn't matter` });
      return true;  // non-back-button was found
    },
