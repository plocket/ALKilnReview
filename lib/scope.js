...
module.exports = {
  ...
  tap_with_promises: async function(scope, { elem, click_promise }) {
    ...
    try {
      ...
      if ( error.name === `TimeoutError` ) {
        ...
        let err_msg = `An error occurred when ALKiln tried to tap "${ elemText }".`
  ...
  // tap_navigation_elem
  after_tap: async function(scope, {}) {
    /** Clean up and validate after tapping. This must happen only
     *  when navigation is attempted. */
    if ( !scope.page ) { return scope; }

    // Start waiting for things before we try to click so we can catch them
    // in time

    // Wait for daPageLoad with page.evaluate

    // Wait for response from server page.evaluate or page.waitForResponse in a loop.
    ...
    /*
      Solve:
        Wait for response
        Wait for jQuery event `daPageLoad`
    */
    ...
    */
    ...
    /*
    * Nav:
    *   1. Successful nav: 200, daPageLoad + new target var (if target var exists)
    *       The first 2 are not sufficient.
    *   2. Fail infinite page: 200, daPageLoad + same consecutive trigger var (and maybe same url, but maybe not) (infinite page)
    *   3. Fail with more complex loop: repeat non-consecutive trigger vars ~page id repeated too many times~
    *       Same target var and page id? Maybe not a lot more signal because of review screens/editing. More complexity.
    *   4. Failed system error: 501/5nn, error DOM (not reliable)
    *   5. Failed simple invalid answer: no 200, no request, no daPageLoad -
    *       how long to wait? (look for invalid elements, but DOM can be changed.
    *       Maybe that's on them.)
    * No nav:
    *   ~~Success no nav: No way to know~~
    *   6. ?? Failed no nav: 501/504 (but how long do we wait) - detect a request,
    *       which should happen right away, to expect a response? with a timeout.
    *       daPageLoad? Can't check in browser.
  ...
  set_prev_trigger_var: async function( scope, { scenario }) {
     ...
     *  Also, good name. */
    scenario.previous_trigger = await scope.get_trigger_var( scope );
  ...
  //   // Do we need to count separately for each "user"? For each Story Table?
  //   let trigger = await scope.get_trigger_var( scope );
  ...
  get_trigger_var: async function() {
