...
module.exports = {
  ...
  getField: async function ( scope, { var_name, choice_name, set_to }) {
    ...
    // If an index var was found
    if ( index_name !== var_name ) {
      var_name = index_name;
      base64_var_name = await scope.base64( scope, index_name );
      base64_id_of_first_choice = base64_index + '_0';
      ({ name_attr_B, name_attr_R } = await scope.checkboxNameAttribute( scope, index_name, choice_name ));
    }
    ...
    let selectors = [
      ...
      `*[data-saveas="${ base64_var_name }"] input:not([type="checkbox"])`,  // text input showif
      ...
      `input[name="${ name_attr_B }"]`,  // checkbox (other)
      `input[name="${ name_attr_R }"]`,  // checkbox (other) alt
    ...
    let handle = await scope.page.evaluateHandle(({
      ...
      // <div data-saveas="encoded_var_name">...<input name="encoded("_field_n[B/R'encoded_choice_name']")">

      // Showif checkbox other. It uses things like `_field_1[B'encodedStr']` instead of var name.
      // That `_field_n` can be found in the input `id`, which is of the format `encodedStr_n`.
      // For example: `X2ZpZWxkXzM_0`. `n` is a count of the number of field groups hidden under the showif.
      let showif_parent_selector = `*[data-saveas="${ base64_var_name }"]`;
        let showif_parent = document.querySelector( showif_parent_selector );
        if ( showif_parent ) { console.log( 'showif_parent for', var_name, choice_name, showif_parent ); }

      try {
        // Let's get pythonic and not use a ton of `if` statements
        // First get the field group's first child so we can get the right `_field_n`

        // Get the id and remove the trailing `_n` so it can be decoded
        let showif_base64_id_of_first_choice = showif_parent.querySelector('input').id.replace(/_.*/, '');
        if ( showif_base64_id_of_first_choice ) { console.log( 'showif_base64_id_of_first_choice for ', var_name, choice_name, showif_base64_id_of_first_choice, showif_parent.querySelector('input') ); }
        // Decode the `_field_n`
        let id = window.atob( showif_base64_id_of_first_choice );
        console.log( 'decoded id', var_name, id );
        // Encode possible choice values, removing any trailing `==` that `atob` adds
        let encoded_choice = window.btoa( choice_name ).replace(/=+/, '')
        // ex: "_field_3[B'c2hvd2lmX2NoZWNrYm94ZXNfbm90YV8y']"
        let encoded_value_B = `${ id }[B'${ encoded_choice }`;
        let encoded_value_R = `${ id }[R'${ encoded_choice }`;
        console.log( 'encoded choice', encoded_value_B, encoded_value_R );
        // See if the value exists
        let possible_node = document.querySelector( `${ showif_parent_selector } input[name="${ encoded_value_B }"]` );
        if ( possible_node ) {
          console.log( 'B:', choice_name, possible_node );
          return possible_node;
        } else {
          possible_node = document.querySelector( `${ showif_parent_selector } input[name="${ encoded_value_R }"]` );
          if ( possible_node ) { console.log( 'R:', choice_name, possible_node ); }
          if ( possible_node ) { return possible_node; }
        }

      // Skip an error. We can't see it anyway.
      } catch (err) {}

      // // Do the same for simplest index var case
      // showif_parent_selector = `*[data-saveas="${ base64_index }"]`;
      // showif_parent = document.querySelector( showif_parent_selector );
      // showif_base64_id_of_first_choice = showif_parent.querySelector('input').id.replace(/_.*/, '');
      // id = window.atob( showif_base64_id_of_first_choice );
      // encoded_choice = window.btoa( choice_name ).replace(/=+/, '')
      // // "_field_3[B'c2hvd2lmX2NoZWNrYm94ZXNfbm90YV8y']"
      // encoded_value_B = `${ id }[B'${ encoded_choice }`;
      // encoded_value_R = `${ id }[R'${ encoded_choice }`;

      // let possible_node = document.querySelector( `${ showif_parent_selector } input[value="${ encoded_value_B }"]` );
      // if ( possible_node ) { return possible_node; }
      // else {
      //   possible_node = document.querySelector( `${ showif_parent_selector } input[value="${ encoded_value_R }"]` );
      //   if ( possible_node ) { return possible_node; }
      // }
      ...
      for ( let selector of selectors ) {
        ...
        try {
          ...
          if ( possible_node ) { console.log( 'standard:', selector, possible_node ); }
          if ( possible_node ) { return possible_node; }
      ...
      // TODO: Priority 1
      // ask C if she's got hidden checkboxes and to avoid testing those right now
      // then develop more robust methodology
      // Return data contains attributes and selectors. Harder to test nodes if we return nodes.
    ...
    // here are data representations of all fields - node, tags, attrs., decoded
    // do any of our fields match
