...
module.exports = {
    ...
    *  TODO: Pass in `$` instead? */
    ...
    if ( !encoded_trigger_var ) {
      ...
      await scope.addToReport(scope, { type: `warning`, 
          value: `You are missing an element containing the data for the page's trigger variable. `
              + `Some fields, like loops with index variables, may not work correctly in these `
              + `tests. See https://suffolklitlab.org/docassemble-AssemblyLine-documentation/docs/automated_integrated_testing/#a-missing-trigger-variable.` });
    ...
    */
    ...
    for ( let guess of field.guesses ) {
      for ( let table_row of var_data ) {
        ...
        if (names_match) {
          console.log(`${ guess.var } and ${ table_row.var} match?: ${ names_match}.`)
        }
        else if (values_match) {
          console.log(`${ guess.value } and ${ table_row.value } match?: ${ values_match }`)
        } else {
          console.log(`Nothing matched ${ guess.var } and ${ guess.value }`); 
        }
  ...
  uploadFiles: async function ( scope, { handle, set_to }) {
    ...
    try {
      ...
      console.log(`Couldn't access ${ dir_source }`);
      try {
        fs.accessSync( dir_features );
        dir = dir_features;
    ...
    if ( paths.length === 0 ) {
      const msg = `Could not find "${set_to}" in ${ dir }.`;
    ...
    */
    ...
    for ( let var_datum of var_data ) {
      ...
      // Neutralize a `.there_is_another` row in a story table
        ...
        await scope.addToReport( scope, { type: `warning`, 
            value: `The attribute \`.there_is_another\` is invalid in story table tests. Replace `
                + `it with \`.target_number\` in your \`var\` and \`trigger\` columns. Set the `
                + `\`value\` to the number of items in that list. This test will now set this `
                + ` row's \`value\` to \`False\`. See https://suffolklitlab.github.io/docassemble-AssemblyLine-documentation/docs/automated_integrated_testing#there_is_another-loop.`
    ...
    if ( value.match( /(false|no|unchecked|uncheck|deselected|deselect)/i )) {
      ...
      if ( from_story_table ) {
        ...
        if ( row.var.match( /\.there_is_another$/ ) ) {
          await scope.addToReport( scope, { type: `warning`, 
              value: `The attribute \`.there_is_another\` is invalid in story table tests. `
                  + `Replace it with \`.target_number\` in your \`var\` column. Set the \`value\` `
                  + `to the number of items in that list. This test will now set this row's `
                  + `\`value\` to \`False\`. See https://suffolklitlab.github.io/docassemble-AssemblyLine-documentation/docs/automated_integrated_testing#there_is_another-loop. `
