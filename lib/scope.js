  ...
  tap_with_promises: async function(scope, { elem, click_promise }) {
    ...
    try {
      ...
      if ( error.name === `TimeoutError` ) {
        ...
        reports.addToReport( scope, { type: `error`, code: `ALK0104`, value: error.name });
        throw error;
...
// .catch(function (error) {
//   if (error.name === 'AbortError') {
//     console.log('\ndom loaded AbortError\n');
//   } else {
//     console.log('\n\ndom loaded error:', error.name)
//   }
// });

    // page.waitForNavigation isn't enough. The DOM loads, but
    // docassemble emits the `daPageLoad` event theoretically once
    // the js and such are finished loading. That said, it doesn't
    // account for CustomDataType js and such. Maybe we should add
    // an event that those can emit if they want to integrate with
    // ALKiln. Or maybe a Step that will listen for a specified event.
    ...
    // //   resolve(foo);
    // // });
    // await time.waitForTimeout(500);

    // await time.waitForTimeout(1000);

    console.log(`PRE CLICK PROMISE`)
    let click_promise = elem.click()
    .then(async (result) => {
      // await time.waitForTimeout(1000);
      console.log(`> clicked`);
    ...
    .catch((error) => { console.log(`click error:`, error) });
    console.log(`POST CLICK PROMISE`)

    // await time.waitForTimeout(1000);


    // Just return the promise?
    // Take care of timeout error?
    let winner = await Promise.race([
      ...
      Promise.all([ `server_response`,  click_promise, daPageLoad_promise, /*server_reponse_promise, dom_loaded_promise,*/ /*validation code element, we hope*/ ]),  // #nav1, #nav2, #nav3, ~~#nav6 (validation should be here already if we need it)~~
    ...
    // try {
      ...
      await scope.page.evaluate(function abort_daPageLoad( window_key ) {
        ...
        // }
      }, window_key ).then(()=>console.log(110)).catch(()=>console.log(120));
    // } catch (error) {
  ...
  get_daPageLoad_promise: function( scope ) {
    /** Given scope (which contains `page`) create a promise
     *  that listens for the `daPageLoad` event, both of which
     *  can then be aborted/turned off when needed. Return
     *  the promise and the random key that gives access to the
     *  `window.waiters` object to allow future code to abort.
     *
     * Notes:
     *  We avoid assigning directly to `window` to avoid overwriting
     *  something important (e.g. window.document)
     * */
    ...
    const daPageLoad_promise = scope.page.evaluate(( window_key ) => {
      ...
      return new Promise(function ( resolve, reject ) {
        ...
        window.waiters[ window_key ].handler = async function ( event ) {
          window.waiters[ window_key ].event_happened = true;
          await sleep(0);  // This solves issues with the message queue/frame stack
  ...
  },  // Ends scope.get_daPageLoad_promise()
