...
module.exports = {
  ...
  ms_till_server_timeout = 120 * 1000;  // 4 times default scope.timeout
  track_server_status: async function( scope ) {
    ...
    *    eventually, it is told to stop. */
    ...
    if ( !scope.stop_tracking_server_status ) {
      // No await. That's already happened with the server response await
  ...
  wait_for_server_response: async function( scope ) {
    ...
    */
    console.log( 1 );
    scope.ms_till_server_timeout = scope.timeout * 4;
    let server_promise = new Promise(async function( resolve, reject ) {
      ...
      (async function() {
        ...
        while ( scope.server_statuses[0] !== true ) {
          ...
          // I _think_ this reset's puppeteer's timeout so it won't timeout early
          // I'm not sure how long to tell it to wait for. Also, don't want to
          // actually `await` it because I _think_ it'll mess up the timing on
          // everything else. I'm also not sure we want to reset the timeout to
          // scope.timeout again because that's not how long the server reload
          // cycle will wait for. For now we'll try this. The default timeout will
          // be reset to scope.timeout when the next interview is loaded (which is
          // when scope.page will next exist), so we don't have to reset it in here
          scope.page.waitForTimeout( 1 );
  ...
  //   */
  //   if ( scope.server_reload_promise ) {
  ...
  throw_server_was_reloading_error: async function( scope ) {
    /* Does what needs to be done when the server reload fails. */
    ...
    // Ensure puppeteer will also timeout and error. I think. Otherwise
    // the test's process might not exit appropriately. Try to get it to happen
    // shortly after the error is thrown.
    if ( scope.page ) {
      await scope.page.setDefaultTimeout( 5 );
    }
      ...
      */
      ...
      if ( error.name === `TimeoutError` ) {
        ...
        if ( server_reloaded_during_interval ) {
          // Set page's defaultTimeout to the time it'll take for the server check
          // to complete. I'm avoiding resetting the timeout to scope.timeout again
          // later in here because that's not how long the server reload
          // cycle will wait for. For now we'll try this. The default timeout will
          // be reset to scope.timeout when the next interview is loaded (which is
          // when scope.page will next exist), so we don't have to reset it in here
          await scope.page.setDefaultTimeout( scope.ms_till_server_timeout );
          // I _think_ this reset's puppeteer's timeout so it won't timeout early
          // I'm not sure how long to tell it to wait for. Also, don't want to
          // actually `await` it because I _think_ it'll mess up the timing on
          // everything else.
