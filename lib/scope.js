...
module.exports = {
  ...
  examinePageID: async function ( scope, question_id = '' ) {
    // Refactoring:
    // Problem - you have to know a lot about this function to understand
    // what you're getting from it. Also, mostly not all this functionality
    // is used in one place. Maybe break it up.
    ...
    *     re.sub(r'[^A-Za-z0-9]+', '-', interview_status.question.id.lower()) */
    ...
    // dom_id = get_DOM_question_id( scope );
    // page_id = convert_to_HTML_question_id( scope, { question_id });
    // return { question_has_id: dom_id !== null, id: dom_id, id_matches: dom_id === page_id };
  ...
  // Cat: DOM discovery (async, unless we use cheerio, which we could...? scope.html?)
  get_DOM_question_id: async function ( scope ) {
    /* Returns the HTML value in the DOM that the YAML question block id
    * got transformed into by docassemble. */
    let classes_str = await scope.page.$eval(`body`, function (elem) { return elem.className });
    let ids_found = classes_str.match(/question-([^ ]+)/);
    if ( ids_found ) { return ids_found[1]; }
    else { return null; }
  },

  // Cat: align_strings_with_da
  convert_to_HTML_question_id: async function ( scope, { question_id='' } = {}) {
    /* Convert the YAML version of the question block id into the one that
    * docassemble puts into the DOM.
    * re.sub(r'[^A-Za-z0-9]+', '-', interview_status.question.id.lower()) */
    return question_id.toLowerCase().replace(/[^A-Za-z0-9]+/g, '-');;
  },
  ...
  processVar: async function ( scope, { var_data, id, from_table=true }) {
    ...
    * WARNING: 'When I tap to continue' should AVOID this function. */
    ...
    // the screen may be revealed and then need to be set.
    ...
    let something_was_set = true;
    while ( something_was_set && fields_to_set.length > 0 ) {
      ...
      for ( let field_i = 0; field_i < fields_to_set.length; field_i++ ) {
        ...
        // We will only continue/navigate after setting all other possible fields (see next loop)
        ...
        if ( row_used ) {
          ...
          // ??: Should this be an `else`? Should it happen always?
    ...
    let already_continued = false;  // ??: `navigated_to_the_next_page_after_setting_fields_by_setting_another_field_thats_in_the_table`
    let error = {};
    for ( let data_i = 0; data_i < fields_to_set.length; data_i++ ) {

      let { tag, type } = fields_to_set[ data_i ];
    ...
    // ??: from_table -> setting_all_fields_on_this_page, setting_multiple_fields
    ...
    if ( from_table && !already_continued ) {
      ...
      // generator - interesting, but probably terrible, idea
