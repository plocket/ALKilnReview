...
module.exports = {
  ...
  setCustomDatatype: {
    al_date: async function ( scope, { handle, answer, is_past }) {
      ...
      * field values. */
      ...
      // == debug
      console.log(`Wait for date: ./${ scope.paths.scenario }/_3_parts_date-${ Date.now() }.jpg`);
      // First wait because we've found it takes a while for a custom date to
      // populate its custom inputs
      await page.waitForTimeout(1000 * 10);
      await scope.page.screenshot({
        path: `./${ scope.paths.scenario }/_3_parts_date-${ Date.now() }.jpg`,
        type: 'jpeg',
        fullPage: true,
      });
      // == ends debug
  ...
  enter_answer: {
    ...
    select: async function ( scope, { handle, answer }) {
      ...
      console.trace(`calling select with ${ answer }`);
      if ( option ) {
        console.log(`the option wasn't encoded`);
      ...
      // TODO: Can these be double encoded? Can they be encoded like objects?
        ...
        console.log(`the option was encoded`);
