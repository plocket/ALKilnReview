...
module.exports = {
  ...
  tapElementAndNavigate: async function(scope, { elem, waitForTimeout=0 }) {
    ...
    scope.set_expected_navigation_state(scope, { expected: true });

    let start_url = await scope.page.url();  // so we can later check for navigation

    await scope.tap_with_promises(scope, {
      elem,
    ...
    // with a user input server validation error or infinite loop. Does that
    // count as navigating? Use trigger var as well?
    let end_url = await scope.page.url();
    let navigated = start_url !== end_url;
    scope.set_navigated_state( scope, { navigated });
    ...
    if ( navigated ) {
      ...
      await scope.afterStep(scope, { waitForTimeout: 200, navigated });
    } else {
      // let code = `ALK0205`;
      // let msg = `ALKiln should be on a new interview page, but it is on the same page. Did you mean to use the Step "I tap the "<selector>" element and stay on the same page"?`;
      // reports.addToReport(scope, { type: `error`, code: code, value: msg });
      // throw new Error(`🤕 ${ code } ALKiln ERROR: ${ msg }`);
  ...
  tapElementAndStayOnPage: async function(scope, { elem }) {
    /** Tap a given element. Assume the user knows what they're doing and
     *  that the page doesn't navigate. There's no way for us to
     *  know when such a tap is successful.
     *
     * We are using this to tap tabs, terms, and random selectors
     * (like expandable content).
     */
    // scope.set_navigated_state(scope, { navigated: false });
    // scope.set_expected_navigation_state(scope, { expected: false });

    // Do we want to check that this really doesn't navigate? How?
    // What else can go wrong? Maybe detect system error? Or maybe do
    // that after/before each Step.

    // Failed no nav: 501, error elements (but how long do we
    // wait) - detect a request to expect a response?

    await scope.guard_against_nonexistant_tap_element( scope, { elem });
    // await elem.click();

    // let click_promise = elem.click();
    // await scope.tap_with_promises(scope, { elem, click_promise });
  },  // Ends scope.tapElementAndStayOnPage()

  // TODO: consider making this a general check for an element with a custom message?
  guard_against_nonexistant_tap_element: async function(scope, { elem }) {
    /** Check that an element that we need to tap does exist. If not,
     *  throw an error. */
  ...
  tap_with_promises: async function(scope, { elem, click_promise }) {
    /** Tap an element and see what we can detect about what happens.
     *
     * Nav:
     *   Successful nav: 200, daPageLoad + new target var (if target var exists)
     *       The first 2 are not sufficient.
     *   Fail infinite page: 200, daPageLoad + same consecutive trigger var (and maybe same url, but maybe not) (infinite page)
     *   Fail with more complex loop: repeat non-consecutive trigger vars ~page id repeated too many times~
     *       Same target var and page id? Maybe not a lot more signal because of review screens/editing. More complexity.
     *   Failed system error: 501/5nn, error DOM (not reliable)
     *   Failed simple invalid answer: no 200, no request, no daPageLoad -
     *       how long to wait? (look for invalid elements, but DOM can be changed.
     *       Maybe that's on them.)
     * No nav:
     *   ~~Success no nav: No way to know~~
     *   ?? Failed no nav: 501/504 (but how long do we wait) - detect a request
     *       to expect a response? with a timeout
     * */
    await scope.guard_against_nonexistant_tap_element( scope, { elem });
    ...
    try {
      ...
      //   // These are meant to detect user-visible alerts/errors,
      //   // not breaking errors in the testing code.

      //   // `validation code`
      //   scope.page.waitForSelector('.alert-danger', { visible: true }),
      //   // Field invalid input element
      //   scope.page.waitForSelector('.da-has-error', { visible: true }),
      // ]);

      winner = await scope.get_nav_attempt_winner( scope, { elem });
      console.log( JSON.stringify(winner) );

    } catch ( error ) {

      let elemText = await (await elem.getProperty('textContent')).jsonValue();
      if ( error.name === `TimeoutError` ) {
        let non_reload_report_msg = `After ALKiln tapped "${ elemText }" it took too long to load the next page.`;
        ...
        let err_msg = `An error occurred when ALKiln tried to tap "${ elemText }". See pictures for more details.`
  ...
  },  // Ends scope.tap_with_promises()

  // TODO: Rename to after_tap (checks for system error after interaction)
  // TODO: Maybe just `wait_for_showif`?
  ...
  tap_navigation_elem: async function(scope, { elem }) {
    /** Tap an element that is able to navigate and deal with the consequences. */
    if ( !scope.page ) { return scope; }

    await scope.guard_against_nonexistant_tap_element( scope, { elem });

    // try {
    //   let winner = await get_nav_attempt_promises( scope, { elem });
    // } catch ( error ) {
    //   // timeout_error()
    //   // or other_error()
    // }
    ...
    // Another way to fail page navigation
    let met_navigation_expectations = scope.does_navigation_state_match_expectations({ scenario });
    if ( !met_navigation_expectations ) {
      await scope.throw_navigation_error( scope, { scenario });
    } else {
      log.debug({
        code: `ALK0206`, type: `info`,
        pre: `Navigation did match expectations. Should have navigated: ${ nav.expected }. Did navigate during the last Step: ${ nav.actual }.`
      });
    }

    // TODO: Check that navigation wasn't a system error
    scope.reset_navigation_states({ scenario });

    // Possibly throw errors

    // Check repeat trigger var

    // TODO: names: validate, constrain
    await scope.increment_page_id( scope, { pages: scenario.pages });
    await scope.guard_against_inifinite_page_loop( scope, { scenario });
    scenario.prev_page_id = current_page_id;

    return scope;

    /*
    click, detect request, detect response, detect navigation
    */

    /*
    winner = await Promise.race([
      ~~Promise.all([click_promise, page.waitForNav]),  // not enough~~

      // Add click to all these with Promise.all()?
      // Add request wait to responses?

      // # 1, 2, 3, 4
      // daPageLoad (how?? polling?)
        // 200
          // target var (simple and complex)
        // 501 (5nn)
    ...
    // On timeout, what do we do????

    [Yes] Maybe this really is just after tapping navigation elem (even if
    then expect to fail to navigate). Maybe taps that don't interact with
    navigation elements don't get any checks?

    Wait

    Winner (no throwing yet?):
      Error elem (200 or not):
        invalid, no_nav
        break
      200:
        (error elem was taken care of already)
        #2 consecutive repeat, no_nav
        #3 infinite loop, no_nav
        #1 did_navigate
      #4 501:
        system error, no_nav
        throw error
    ...
    */
    ...
    /*
    // response catcher
    System error (501? 504? 5nn?)? (response stack?)
      throw

    increment page id?

    Did nav? (note all these?)
      click/wait to navigate? Consistent? What if not navigating?
      `daPageLoad` listener
      different trigger var
      different page id
      status code 200 (and 305?)? happens with invalid values and infinite pages, maybe useless
      different url? (inform infinite loop vs. invalid values? Necessary?)
    No nav?
      never sent request (request stack?)
      invalid answer (stayed)

    progress trigger var

    If !nav expectations met:
      If stayed unexpectedly:
        why stayed?
          invalid answer?
            throw
          same trigger?
          ...
          infinite page?
            throw
      If navigated unexpectedly:
        throw with current page & trigger var?
        throw with "should not have passed previous page id/trigger var".
          With current page & trigger var?
          With answers? Those should already be listed.
    else:
      ???
    reset nav states (maybe after every Step, not just here?)



    throw if repeat trigger
    throw if infinite page
    */


    // Unexpected failed navigation - not necessarily infinite loop.
    // Could be invalid input. Check for such?
  },

  // Wait for daPageLoad with page.evaluate?
  get_nav_attempt_winner: async function( scope, { elem }) {
    /** Wait for navigation indicators or lack thereof, then return the result.
    *
    * Nav:
    *   #nav_1 Successful nav: 200, daPageLoad + important page differences or new target var (if target var exists)
    *       The first 2 are not sufficient alone.
    *   #nav2 Fail infinite page: 200, daPageLoad + same consecutive trigger var (different url) (infinite page)
    *   #nav3 Fail with more complex loop: 200, daPageLoad + repeat non-consecutive trigger vars ~page id repeated too many times~
    *       Same target var and page id? Maybe not a lot more signal because of review screens/editing. More complexity.
    *   #nav4 Failed system error: 501/5nn, error DOM (not reliable)
    *   #nav5 Failed simple browser-side invalid answer: no 200, no request, no daPageLoad -
    *       how long to wait? (look for invalid elements, but DOM can be customized
    *       (rarely on that level of detail, so maybe that's on them.))
    *   #nav6 Fail with server-side validation code: 200, daPageLoad + same target var and url
    *   #nav7 305 for successful sign in?
    * No nav (ignoring for now):
    *   ~~Success no nav: No way to know~~
    *   ?? Failed no nav: 501/504 (but how long do we wait) - detect a request,
    *       which should happen quickly if it happens, to expect a response?
    *       With a short timeout. That doesn't block other things.
    *
    * ## Lifecycles
    * Navigation:
    *   1. click
    *   2. waitForNavigation, 'domcontentloaded'
    *   3. daPageLoad
    *   4. Race over
    * Stay:
    *   1. client-side element
    *   2. click
    *   3. Race over
    *  */

    // ADD CHECKING THE URL IN CASE THERE'S NO PAGE ID AND NO TARGET VARIABLE

    // Start waiting for things before we try to click so we can catch
    // the events in time

    // // #nav1, #nav2, #nav3, #nav4, #nav6, #nav7
    // let response_promise = scope.page.waitForResponse(function (response) {
    //   console.log( response );
    //   // Example Playground request url:
    //   // server_url/interview?i=docassemble.playgroundIDProject:file.yml
    //   // Example server interview request url:
    //   // server_url/interview?i=docassemble.dispatch_name:data/questions/file.yml
    //   // I'm not sure this `interview?` string is ironclad.
    //   let is_right_url = /http.*\/interview\?.*\.yml/.test(response.request().url());
    //   let status = response.status();
    //   return is_right_url && (
    //     response.ok()  // #nav1, #nav2, #nav3, #nav6 (200 - 299)
    //     || (status >= 500 && status < 600)  // #nav4, system error
    //     || status === 305  // #nav7, sign in redirect
    //   );
    // });

    // Check request url to make sure we're returning a relevant response
    // Example Playground request url:
    // server_url/interview?i=docassemble.playgroundIDProject:file.yml
    // Example server interview request url:
    // server_url/interview?i=docassemble.dispatch_name:data/questions/file.yml
    ...
    // Abort all promises after winning the race
    // const abortController = new AbortController();
    // const mission_status =

    let controller = new AbortController();
    let mission_status = controller.signal;

    // let daPageLoad_promise = scope.page.evaluate(async function() {
    //   // return new Promise(function ( continue_with, throw_with ) {
    //   //   $(document).one(`daPageLoad`, function( event ) {
    //   //     console.log('> page load');
    //   //     continue_with( document.body.innerHTML );  // Return HTML?
    //   //   });
    //   // });
    //   await new Promise(function ( continue_with, throw_with ) {
    //     $(document).one(`daPageLoad`, function( event ) {
    //       console.log('> page load');
    //       continue_with( document.body.innerHTML );  // Return HTML?
    //     });
    ...
    // TODO: IMPORTANT! May need the response url instead, not the response.request().url()
    let count = 1;
    let ok_id = 200;  // #nav1, #nav2, #nav3, #nav6 (200 - 299)
    let server_reponse_promise = scope.page.waitForResponse(function ( response ) {
      // Method can be POST or GET, so that's not particularly helpful.

      let status = response.status();
      console.log( `> status/response url:`, status, `/`, response.url(), `\n`, response.request().url() );

      // 500s & 300s
      // Question: What does it mean if a 500 has a non-interview url?
      if ( `${status}`.startsWith(`5`) || `${status}`.startsWith(`3`) ) { return response; }

      // Valid 200s must be interview urls. Otherwise, try again.
      // response.url()?
      let is_interview_url = /http.*\/(?:interview|run|start)\?.*\.yml/.test(response.request().url());
      if ( !is_interview_url ) { return false; }

      if ( response.ok() ) { return response; }

      // // Otherwise we're loading an interview page that will have a `daPageLoad` event
      // // #nav1, #nav2, #nav3, #nav6, #nav7, any page reload
      // // let daPageLoad_promise = new Promise(async function (resolve1, reject1) {
      //   let daPageLoad_promise = scope.page.evaluate(function() {
      //   // await scope.page.evaluate(function() {
      //     return new Promise(function(resolve2, reject2) {
      //       $(document).on(`daPageLoad`, function(event) {
      //         console.log('> load event');
      //         resolve2(true); // resolves when the event fires
      //       });
      //     });
      //   // })
      //   //, { signal: mission_status });
      // //   console.log( `> page load` );
      // //   resolve(foo);
      // }).then(() => console.log(`> page load`))
      // .catch(function (error) {
      //   if (error.name === 'AbortError') {
      //     console.log('\n\ndaPageLoad AbortError: Fetch request aborted');
      //   } else {
      //     console.log('\n\ndaPageLoad:', error.name)
      //   }
      // });
    ...
    .then(async function ( response ) {
      ...
      // I guess this often comes too late

      // console.log( `\n=======\n=======\n`, await response.json() );
      // let body = response.json().body;
      // console.log(`body:`, body[0], body[1], body[2], body[3], body[4], body[5], body[6] );
      // console.log(`body class:`, response.json().bodyclass );

      let data = {
        status: response.status(),
        url: response.request().url(),
        // expect_da_page_load: false,
        // da_page_loaded: false,
        // We can't detect custom HTML, like with some CustomDataTypes
        // TODO: discuss adding a 'wait for selector' Step?
        json_promise: response.json(),
        // html: response.json().body,
        // body_class: response.json().bodyclass,
      };

      // 300s & 500s (error) are fine as they are. 300s could theoretically
      // have a daPageLoad event if they go to an interview, but for now
      // we are using them only for the simplest of cases.
      // Note: Either could have an interview url
      if ( !response.ok() ) { return data; }

      // // Just 200s. To get in here, it must have been an interview url
      // // We hope interview urls that got a 200 must always have a
      // // daPageLoad event. Wait for it.
      // data.expect_da_page_load = true;
      // if ( data.expect_da_page_load ) {
      //   console.log(`=== waiting for page load ${ count }:`, data);
      //   count++;
      //   return scope.page.evaluate(function( data ) {
      //     return new Promise(function(continue_with, reject) {
      //       $( document ).on( `daPageLoad`, function( event ) {
      //         data.da_page_loaded = true;
      //         continue_with( data );
      //       });
      //     });
      //   }, data )
      //   .then(( data ) => {
      //     console.log(`done waiting for daPageLoad event ${ count }`); return data;
      //   });  // ends page.evaluate
      // } else {
        return data;
      // }
      ...
      if (error.name === `AbortError`) {
        console.log(`Navigation AbortError: Fetch request aborted`);
      } else {
        console.log( `Navigation other error:`, error.name );
      }
    });

    /**
    Pseudo code:
    Promise in page.evaluate
      event_happened = false
      named_handler = func
      $(‘event’).on(named_handler:
         event_happened = true
         resolve(true)
    page.watiForFunction( function:
      return event_happened
    .catch(error
      if error is Abort
      page.evaluate(removeEventListener)

    Refined/robusted with @cthulahoops:
    function waitForEvent(page, eventName, {signal}) {
        const promise = page.evaluate(
            (eventName, timeout) => {
                window.abort_controller = AbortController()
                window.event_happened = false
                new Promise(
                    function (resolve, reject) {
                        window.func = (event) => {
                            window.event_happened = true;
                            resolve(event);
                        }
                        document.addEventListener(eventName, window.func, {once: true})
                    }
                )
            }, eventName, timeout
        )
        ...
        page.waitForFunction(
                () => { return window.event_happened; }, {signal}
            ).catch((err) =>
                {
                    if (err.name === "AbortError") {
                        page.evaluate(() => {
                            document.removeEventListener(eventName, window.func);
                            window.abort_controller.signal.abort();
                        }).catch(() => "No one cares")
                    } else {
                        // Other exceptions?
                    }
    ...
    // This won't work for multiple calls because the shared global
    // variables (window.func and window.abort_controller) will get changed:
    //
    // Promise.race( [waitForEvent(...), waitForEvent(...)] )
    //
    // Perhaps pass in a name:
    //
    // window.wait_things[name].abort_controller = ...
    // window.wait_things[name].func = ...
    // window.wait_things[name].event_happened = ...
    //
    // Don't assign to random names on window because you might accidentally
    // overwrite something important. (eg. window.document)
    */

    //   /** Listen for daPageLoad only after verifying the response since
    //    *  this cannot be aborted. Thank you @willmorningstar! */

    //   // An interview page has a `daPageLoad` event
    //   if ( /http.*\/interview\?.*\.yml/.test(response.url()) ) {
    //     // todo
    //   }

    //   let da_page_html = await scope.page.evaluate(function() {
    //     return new Promise(function ( continue_with, throw_with ) {
    //       $(document).one(`daPageLoad`, function( event ) {
    //         console.log('> load event');
    //         continue_with( document.body.innerHTML );  // Return HTML?
    ...
    //   console.log(da_page_html);

    //   if ( da_page_html && da_page_html.includes(`<html>`) ) {
    //     console.log( `200 response with HTML` );
    //     return {
    //       url: response.url(),
    //       status: response.status(),
    //       // Need html to look for `validation code` elem
    //       html: da_page_html,
    //     }
    //   } else {
    //     throw new Error(`HTML is missing for 200 response`);
    //   }
    ...
    // // Check daPageLoad?
    // let system_error_id = 500;  // #nav4, system error
    // let response_500s_promise = scope.page.waitForResponse(function (response) {
    ...
    //   // let is_right_url = /http.*\/interview\?.*\.yml/.test(response.request().url());
    //   let is_right_url = /http.*\/interview\?.*\.yml/.test(response.url());
    //   let status = response.status();
    //   console.log( `> check 500` );
    //   return is_right_url && (status >= 500 && status < 600);
    // }, { signal: mission_status }).catch(function (error) {
    //   if (error.name === 'AbortError') {
    //     console.log('500 AbortError: Fetch request aborted');
    //   } else {
    //     console.log('\n\n500:', error.name)
    //   }
    // });
    ...
    // let redirect_id = 300
    // // Redirect to non-interview page (no daPageLoad on redirect to non-interview page)
    // let redirect_to_server_id = `305 to non-interview`;  // #nav7, sign in redirect
    // // let response_305_non_interview_promise = scope.page.waitForResponse(function (response) {
    // let response_305_promise = scope.page.waitForResponse(function (response) {
    ...
    //   let is_server_url = response.url().startsWith( session_vars.get_da_server_url() );
    //   let is_interview_url = /http.*\/interview\?.*\.yml/.test(response.url());
    //   let status = response.status();
    //   console.log( `> check 305` );
    //   return is_server_url && !is_interview_url && status === 305;
    // }, { signal: mission_status }).catch(function (error) {
    //   if (error.name === 'AbortError') {
    //     console.log('305 non AbortError: Fetch request aborted');
    //   } else {
    //       console.log('\n\n305 non:', error.name)
    //     }
    // });
    ...
    // let redirect_to_interview_id = `305 to interview`;  // #nav7-ish, redirect to interview page
    // let response_305_interview_promise = scope.page.waitForResponse(function (response) {
    ...
    //   let status = response.status();
    //   console.log( `> check 305` );
    //   return is_interview_url && status === 305;
    // }, { signal: mission_status }).then(
    //   /*Listen for daPageLoad. Thank you @willmorningstar!*/
    //   // If url is interview, listen for daPageLoad
    //   // If that times out, that is an appropriate timeout
    //   // otherwise, just return something else useful (or not? Just finish?)
    // ).catch(function (error) {
    //   if (error.name === 'AbortError') {
    //     console.log('305 √ AbortError: Fetch request aborted');
    //   } else {
    //     console.log('\n\n305 √:', error.name)
    //   }
    // });
    ...
    let client_side_id = `client_side_invalid`;  // #nav5
    let invalid_field_promise = scope.page.waitForSelector(
      '.da-has-error',
      { visible: true, signal: mission_status }
    )
    .then((something) => { console.log(`> client-side invalid`); return `client-side validation`; })
    .catch(function (error) {
        if (error.name === 'AbortError') {
          console.log('\n\nclient-side AbortError: Fetch request aborted');
        } else {
          console.log('\n\nclient_side:', error.name)
        }
      });
    // let invalid_field_promise = new Promise(async function (resolve, reject) {
    //   let foo = await scope.page.waitForSelector('.da-has-error', { visible: true });
    //   console.log( `> client-side` );
    //   resolve(foo);
    // });


    let validation_code_id = `server_side_invalid`;  // #nav6
    let validation_code_promise = scope.page.waitForSelector(
      '.alert-danger',
      { visible: true, signal: mission_status }
    )
    .then((something) => { console.log(`> server-side invalid`); return `server-side validation`; })
    .catch(function (error) {
        if (error.name === 'AbortError') {
          console.log('\n\nserver-side AbortError: Fetch request aborted');
        } else {
          console.log('\n\nserver-side:', error.name)
        }
      });
    // let validation_code_promise = new Promise(async function (resolve, reject) {
    //   let foo = await scope.page.waitForSelector('.alert-danger', { visible: true });
    //   console.log( `> server-side` );
    //   resolve(foo);
    // });
    // let validation_code_promise = scope.with_abort(scope, {
    //   promise: scope.page.waitForSelector('.alert-danger', { visible: true }),
    //   mission_status
    // });

    // Abandon?
    // Wait for DOM for everything that will get a response.
    // When does this happen?
    // TODO: Would this really happen before #nav6, validation code? Race condition?

    // let dom_loaded_promise = scope.page.waitForNavigation({
    //   waitUntil: 'domcontentloaded', signal: mission_status
    // });//.then(/*Listen for dapageload*/).catch(/**/);

    let dom_loaded_promise = //new Promise(async function (resolve, reject) {
      // let foo = await scope.page.waitForNavigation({ waitUntil: 'domcontentloaded' });
      scope.page.waitForNavigation({ waitUntil: 'domcontentloaded' }, { signal: mission_status })
      // console.log( `dom loaded` );
      // resolve(foo);
    // })
    .then(function (result) { console.log( `dom loaded` ); return `dom loaded`; })
    .catch(function (error) {
      if (error.name === 'AbortError') {
        console.log('\ndom loaded AbortError: Fetch request aborted\n');
      } else {
        console.log('\n\ndom loaded:', error.name)
      }
    });

    let { daPageLoad_promise, window_key } = scope.get_daPageLoad_promise( scope );

    console.log(`after promise`);

    // // Trigger click last, for all cases
    // let click_promise = elem.click()
    // .then(() => { console.log(`> click`); return `click`; })
    // .catch((error) => { console.log(`click error:`, error) });
    // // let click_promise = new Promise(async function (resolve, reject) {
    // //   let foo = await elem.click();
    // //   console.log( `click` );
    // //   resolve(foo);
    // // });
    console.log(`PRE CLICK`)
    let click_promise = await elem.click().catch((error) => { console.log(`click error:`, error) });
    console.log(`POST CLICK`)


    // Just return the promise?
    let winner = await Promise.race([
      // Page load event happens after response, but we need the response data
      // I believe we get them back in the same order
      // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/all
      // Promise.all([ click_promise, response_promise, daPageLoad_promise ]),  // #nav1, #nav2, #nav3, #nav4, #nav6, #nav7
      // Promise.all([ system_error_id,    click_promise, response_500s_promise, /*daPageLoad_promise,*/ dom_loaded_promise ]),  // #nav4
      // Promise.all([ redirect_id,        click_promise, response_305_interview_promise, /*daPageLoad_promise,*/ dom_loaded_promise, ]),  // #nav7
      // Promise.all([ validation_code_id, click_promise, server_reponse_promise, /*daPageLoad_promise,*/ dom_loaded_promise, /*, validation_code_promise*/ ]),  // #nav6 (should finish before dom_loaded_promise below)
      Promise.all([ `server_response`,  /*click_promise,*/ server_reponse_promise, daPageLoad_promise, dom_loaded_promise, /*validation code element, we hope*/ ]),  // #nav1, #nav2, #nav3, ~~#nav6 (validation should be here already if we need it)~~
      Promise.all([ client_side_id,     /*click_promise,*/ invalid_field_promise ]),  // #nav5
    ]);

    console.log( `winner:`, winner );
    // await time.waitForTimeout(5000);

    // Abort in node and on the browser, remove event listeners
    controller.abort();
    const url = await scope.page.url();
    console.log(url);
    try {
      // Abort the Promise in the page and delete the event listener
      await scope.page.evaluate(function abort_daPageLoad( window_key ) {
        // let wait = function () {
        //   new Promise
        // }
        try {
          console.log(7);
          if ( !window.waiters[ window_key ].event_happened ) {
            console.log(8);
            // The event could happen here
            $(document).off( `daPageLoad.${ window_key }` );
            console.log(9);
            // Test whether a double abort errors. Signs point to "no"
            window.waiters[ window_key ].abort_controller.abort();
            console.log(10);
          }
        } catch ( error ) {
          console.log(`off?`, error);
        }
      }, window_key ).then(()=>console.log(11)).catch(()=>console.log(12));
    } catch (error) {
      console.log( `not the new page?`, error)
    }

    return winner;
  },  // Ends scope.get_nav_attempt_winner()

  get_daPageLoad_promise: function( scope ) {
    /** Notes:
     *  This won't quite work because the shared global variables
     *  (window.func and window.abort_controller):
     *
     *  Promise.race( [waitForEvent(...), waitForEvent(...)] )
     *
     *  Perhaps pass in a name:
     ...
     *  window.wait_things[name].event_happened = ...
     *
     *  Don't assign to random names on window because you might
     *  accidentally overwrite something important. (eg.
     *  window.document)
     * */

    const window_key = `daPageLoad_event_${ uuidv4() }`;
    // const abort_controller = new AbortController();
    // const mission_status = abort_controller.signal;

    const daPageLoad_promise = scope.page.evaluate(( window_key ) => {
      // Create browser vars to communicate with later functions
      if ( !window.waiters ) { window.waiters = {}; }
      if ( !window.waiters[ window_key ] ) { window.waiters[ window_key ] = {}; }
      window.waiters[ window_key ].abort_controller = new AbortController();
      window.waiters[ window_key ].mission_status = window.waiters[ window_key ].abort_controller.signal;
      window.waiters[ window_key ].event_happened = false;

      return new Promise(function ( resolve, reject ) {
        console.log(1);
        // If the signal is already aborted, immediately throw to get out
        if ( window.waiters[ window_key ].mission_status.aborted ) {
          reject( window.waiters[ window_key ].mission_status.reason );
        }
        console.log(2);
        // Watch for `abort` signals
        window.waiters[ window_key ].mission_status.addEventListener(`abort`, () => {
          // Throw to stop the main operation, give a reason
          reject( window.waiters[ window_key ].mission_status.reason );
        });
        console.log(3);
        window.waiters[ window_key ].handler = function ( event ) {
          console.log(5);
          window.waiters[ window_key ].event_happened = true;
          console.log(6);
          resolve( `daPageLoad triggered` );
        }
        console.log(4);
        $(document).one( `daPageLoad.${ window_key }`, window.waiters[ window_key ].handler );
        console.log(4.5);
      })
      .then(( daPageLoad_result ) => {
        console.log( daPageLoad_result );
        return daPageLoad_result;
        // console.log(`foo`)
      })
      .catch(function (error) {
        // // TODO: log to verbose logs
        // if (error.name === `AbortError`) {
        //   console.log(`\ndaPageLoad AbortError\n`);
        // } else {
        //   console.log(`\n\ndaPageLoad ${ error.name }:`, error );
        // }
        console.log(`bar`)
      });  // Ends daPageLoad Promise
    }, window_key );  // Ends page.evaluate() daPageLoad

    // // Window doesn't exist in .`waitForFunction`
    // scope.page.waitForFunction(function ( window_key ) {
    //   return window.waiters[ window_key ].event_happened;
    // }, window_key )
    // .catch(function (error) {
    //   if (error.name === "AbortError") {
    //     page.evaluate(() => {
    //       document.removeEventListener(window_key, window.waiters[ window_key ].handler);
    //       window.waiters[ window_key ].abort_controller.signal.abort();
    //     }).catch(() => "No one cares")
    //   } else {
    //       // Other exceptions?
    //   }
    ...
    console.log( `promise?:`, daPageLoad_promise);

    return { daPageLoad_promise, window_key };

    // const window_passer = function ( window ) {
    //   // Create browser vars to communicate with later functions
    //   if ( !window.waiters ) { window.waiters = {}; }
    //   if ( !window.waiters[ window_key ] ) { window.waiters[ window_key ] = {}; }
    //   window.waiters[ window_key ].abort_controller = AbortController();
    //   window.waiters[ window_key ].event_happened = false;

    //   return new Promise(function ( resolve, reject ) {
    //     // If the signal is already aborted, immediately throw to get out
    //     if ( mission_status.aborted ) {
    //       reject( mission_status.reason );
    //     }
    //     // Watch for 'abort' signals
    //     mission_status.addEventListener(`abort`, () => {
    //       // Throw to stop the main operation, give a reason
    //       reject( mission_status.reason );
    //     });
    ...
    //       resolve( event );
    //     }
    //     $(document).one( 'daPageLoad', window.waiters[ window_key ].handler );
    //   });
  ...
  //  */
  // waitForLoad: async function ( scope ) {
  ...
  //             return new Promise(function(resolve, reject) {
  //                 $(document).on(`daPageLoad`, function(e) {
  ...
  //         scope. page.waitFor(scope.timeout)
  //     ]);
  // },

  // with_abort: function (scope, { promise, mission_status }) {
  //   return new Promise(async function( continue_with, throw_with ) {
  //     // If the mission_status is already aborted, immediately throw in order to throw_with the promise.
  //     if ( mission_status.aborted ) {
  //       throw_with(mission_status.reason);
  //     }

  //     // await time.waitForTimeout(1000);

  //     // Perform the main purpose of the API
  //     // Call continue_with(result) when done.
  //     // let result = await promise;
  //     console.log(`What the heck am I doing here?`)
  //     continue_with( result );

  //     // Watch for 'abort' signals
  //     mission_status.addEventListener("abort", () => {
  //       // Stop the main operation
  //       // throw_with the abort reason.
  //       throw_with(mission_status.reason);
  //     });
  //   });
  // },

  // get_no_error_promise: async function(scope, { promise }) {
  //   /** Returns a promise that ensures your promise won't cause an
  //    *  error if it rejects. */
  //   // body...
  // },

  // get_cancellable_promise: function(scope, { promise, cancel }) {
  //   /** Returns a promise that you can cancel with or without throwing an
  //    *  error. You can give it the values to resolve or reject when it gets
  //    *  cancelled.
  //    *
  //    * Based on:
  ...
  //    * */
  //   // Defaults
  //   reject_val = reject_val || `Cancelled the promise and threw an error`;
  //   resolve_val = resolve_val || `Cancelled the promise quietly`;

  //   let cancel = function(resolve, reject) {
  //     if ( do_throw ) { reject( reject_val ); }
  //     else { resolve( resolve_val ); }
  //   };

  //   const new_promise = new Promise(async function (resolve, reject) {
  ...
  //     // });
  //     // console.log('finish');
  //     // resolve(`Promise finished without canceling`);
  //   });

  //   return { promise: new_promise, cancel: cancel };
  // },

  // validate_navigation: async function(scope, {}) {
  //   // body...
  // },

  set_expected_navigation_state: function(scope, { expected = false }) {
    /** Make sure expected navigation gets set correctly in state. */
    let scenario = scope.get_scenario( scope );
    scenario.navigation.expected = expected;
    // console.log(`expected:`, scenario.navigation.expected);
    return scenario.navigation;
  },

  set_navigated_state: function(scope, { navigated = false }) {
    /** Make sure navigation gets set correctly in state. */
    let scenario = scope.get_scenario( scope );
    scenario.navigation.actual = navigated;
    console.log(`navigated:`, scenario.navigation.actual);
    return scenario.navigation;
  },

  override_expected_navigation: function( scope, { override_val }) {
    /** Prepare to override the expected navigation state with a custom one. */
    let nav = scope.get_scenario( scope ).navigation;
    console.log(`nav:`, nav);
    nav.override_the_expected = true;
    nav.override_val = override_val;
    return nav;
  },

  get_navigation_override_value: function( scope ) {
    let nav = scope.get_scenario( scope ).navigation;
    return nav.override_val;
  },

  does_navigation_state_match_expectations: function({ scenario }) {
    /** Return whether the actual navigation state matches the
     *  navigation state that the test expected. */
    let { expected, override_the_expected, override_val } = scenario.navigation;
    let real_expected = expected;
    if ( override_the_expected ) { real_expected = override_val; }
    console.log(`expected/actual:`, real_expected, scenario.navigation.actual);
    return real_expected === scenario.navigation.actual;
  },

  reset_navigation_states: function({ scenario }) {
    /** Make sure navigation gets set correctly in state. */
    scenario.navigation.actual = false;
    scenario.navigation.expected = false;
    scenario.navigation.override_the_expected = false;
    scenario.navigation.override_val = null;
    scenario.navigation.failure_type = null;
    return scenario.navigation;
  },

  increment_page_id: async function( scope, { pages }) {
    /** Add the current page id to the count of how many times that
     *  page id has appeared. `scope.page` must be a valid puppeteer page.
     *
     * WARNING: Be very careful about when we choose to call this to
     *  avoid incrementing a page id for no reason. */

    let current_page_id = await scope.get_page_id( scope );

    if ( current_page_id ) {  // ensure not null, as with no page id or sign in Step
      if ( pages[ current_page_id ] === undefined ) {
        pages[ current_page_id ] = { times_appeared: 0 };
      }
      pages[ current_page_id ].times_appeared += 1;
      return {
        page_id: current_page_id,
        times_appeared: pages[ current_page_id ],
      };
    }  // ends if current_id
    return null;
  },  // Ends scope.increment_page_id()

  guard_against_inifinite_page_loop: async function( scope, { scenario }) {
    /** If there's an infinite page loop, throw an error. */
    // Do we need to count separately for each "user"? For each Story Table?
    let is_infinite = await scope.is_infinite_page_loop( scope, { pages: scenario.pages });
    if ( is_infinite ) {
      // What happens if we don't await this? Async issues?
      await scope.throw_infinite_page_loop_error( scope, { scenario });
    }
  },

  is_infinite_page_loop: async function( scope, { pages }) {
    /** Return whether this is an infinite page loop.
     *
     * If we've visited any one page too many times, we consider it an
     * infinite loop. They should be able to see the id appear multiple
     * times in the report. Note: page id may not exist.*/
    let is_infinite = false;
    let current_page_id = await scope.get_page_id( scope );
    let times_appeared = pages[ current_page_id ] || 0;
    is_infinite = times_appeared > pages.alk_max_same_id_count;

    // TODO: Add to docs that page id may make failing tests fail faster
    // and avoid infinite loops

    return is_infinite;
  },

  throw_infinite_page_loop_error: async function( scope, { scenario }) {
    /** Throw an error that a page appeared too many times.
     *  If navigation is set to fail, that will be a single
     *  occasion and won't interfere with this. */
    let page_id = await scope.get_page_id();
    let msg = reports.addToReport( scope, {
      type: `error`, code: `ALK0206`,
      value: `The page with the id "${ page_id }" has appeared over `
        + `${ scenario.pages.alk_max_same_id_count } times. Docassemble `
        + `may be trying to get a variable that is stuck in a hidden `
        + `\`show if\` field.`
    });
    throw Error( msg );
  },

  set_prev_trigger_var: async function( scope, { scenario }) {
    /** Set the previous trigger. This may get more complicated.
     *  Also, good name. */
    scenario.previous_trigger = await scope.get_trigger_var( scope, { html: await scope.page.content() });
    return scenario.previous_trigger;
  },

  // guard_against_repeat_trigger: async function( scope, { scenario }) {
  //   /** If there's a consecutive repeat trigger var, throw an error.
  //    *  It is ok for trigger vars to reappear non-consecutively.
  //    *  For example, an author might test a review screen, revisiting
  //    *  setting a previous variable, triggering the same variable.
  //    *  The page id infinite page loop check should catch more complex cases. */
  //   // Do we need to count separately for each "user"? For each Story Table?
  //   let trigger = await scope.get_trigger_var( scope, { html: await scope.page.content() });
  //   if ( trigger === scenario.previous_trigger ) {
  //     await scope.throw_repeat_trigger_var( scope, { scenario });
  //   }
  //   return scenario;
  // },

  is_repeat_trigger: async function( scope, { scenario }) {
    let trigger = await scope.get_trigger_var( scope, { html: await scope.page.content() });
    return trigger === scenario.previous_trigger;
  },

  throw_repeat_trigger_var: async function( scope, { scenario }) {
    /** Throw an error that the trigger variable was repeated, meaning the
     *  page couldn't continue. Examples: invalid answers, infinite
     *  page loop. Can we tell the difference between different types?
     *  URL doesn't/does change? */

    let page_id = await scope.get_page_id();
    let msg = reports.addToReport( scope, {
      type: `error`, code: `ALK0207`,
      value: `Docassemble has tried to find the value of the variable `
        + `${ scenario.previous_trigger } on the page with the id `
        + `"${ page_id }" multiple times. The variable may be stuck `
        + `in a hidden \`show if\` field.`
    });

    // TODO: Add to docs that trigger var may make failing tests fail faster
    // and avoid infinite loops

    throw Error( msg );
  },

  get_trigger_var: async function( scope, { html }) {
    /** Return the trigger var element that developers ideally add to
     *  their interviews. AssemblyLine will add it automatically unless
     *  someone overrides the values in their own interview. */
  ...
  throw_navigation_error: async function( scope, { scenario, page_id_count = 0 }) {
    /** Throw an error that navigation didn't happen as expected. */
    let nav_expected = `stayed on the same page`;
    let nav_actual = `went to the next page`;
    if ( scenario.navigation.expected ) {
      let nav_expected = `gone to the next page`;
      let nav_actual = `stayed on the same page`;
    }
    let msg = reports.addToReport( scope, {
      type: `error`, code: `ALK0205`,
      value: `The test should have ${ nav_expected }, but it ${ nav_actual }.`
    });
    throw Error( msg );
  },  // Ends scope.throw_navigation_error()

  get_scenario: function( scope ) {
    return scope.scenarios.get( scope.scenario_id );
  },
  ...
  getAllFields:  async function ( scope, { html }) {
    ...
    *  TODO: Pass in `$` instead? */
    ...
    // Get trigger var
    let trigger = await scope.get_trigger_var( scope, { html });

    // // This is the trigger var element that developers ideally add to their interviews.
    // // AssemblyLine will add it automatically unless someone overrides the values in their own interview.
    // // See the report message to the dev below for a link to an example.
    // let $trigger_elem = $( `#trigger` );
    // if ( !$trigger_elem.length ) { $trigger_elem = $( `#sought_variable` ); }  // Backwards compatibility
  ...
  get_page_id: async function ( scope ) {
    // `.className` is the node's list of classes as a string, separated by spaces
    let id = await scope.page.evaluate(function () {
      if ( window.daQuestionID ) {
        return window.daQuestionID.id;
      } else {
        return null;
      }
    });
  ...
  setFields: async function ( scope, {
    ...
    * @param tap_continue_if_needed {bool} [true] - Default true. If true, presses continue button
    ...
    */
    ...
    if ( !did_navigate && tap_continue_if_needed  ) {
    ...
    }  // ends if !did_navigate && tap_continue_if_needed
  ...
  checkForError: async function ( scope ) {
    ...
    * we're not sure. */
    ...
    // All results are lists to match `$$`
    let possible_errors = {
  ...
  steps: {
    ...
    set_regular_var: async ( scope, var_name, answer ) => {
      ...
      */
      ...
      await scope.setFields( scope, { var_data, tap_continue_if_needed: false, ensure_all_vars_used: true });
    ...
    sign_in: async function ( scope, {
      ...
      */
      ...
      // TODO: make this scenario-based to allow parallel tests
      scope.disable_error_screenshot = true;

      // If there is no browser open, start a new one
      if (!scope.browser) {
        // If debug, show browser (headfully)
        // `new` avoids the new Chrome "headless" mode warning in the console
        let headful = session_vars.DEBUG && `new`;
        let do_headless = !headful;
        scope.browser = await scope.driver.launch({ headless: do_headless });
      }
      if ( !scope.page ) { scope.page = await scope.browser.newPage(); }
  ...
  reportIncludesAllExpected: async function ( scope, { expected=[] }) {
    ...
    */
    ...
    console.log(11.105)
        ...
        excluded.push( one_expectation );
      }
    }

    if ( excluded.length > 0 ) {
      let msg = reports.addToReport(scope, { type: `internal error`, code: `ALK0209`,
        value: `Expected report to include "${ excluded.join(", ") }", `
  ...
  reportDoesNotInclude: async function ( scope, { not_expected=[] }) {
    ...
    */
    ...
    let unexpected = [];
    for ( let unanticipated of not_expected ) {
      if ( report.includes( unanticipated )) {
        // none_included = false;
        // expect( report ).not.to.contain( unanticipated );
        unexpected.push( unanticipated );
      }
    }

    if ( unexpected.length > 0 ) {
      let msg = reports.addToReport(scope, { type: `internal error`, code: `ALK0210`,
        value: `None of these should have been in the report and yet they were: "${ unexpected.join(", ") }". `
          + `Report:\n${ report }`
      });
      console.log( msg );
    }
