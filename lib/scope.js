...
module.exports = {
   ...
   */
    ...
    // TODO: Perhaps use a separate `used_count` prop to compare to. It can be used to inform the report as well.
  ...
  processVar: async function ( scope, { var_data, id, from_table=true }) {
    ...
    * If it's not data in a table, the code can be more permissive about some input. */
    ...
    // Loop through fields until no more fields are getting used up (until nothing is changing) -
    ...
    while ( prev_length > fields_to_set.length && fields_to_set.length > 0 ) {
      ...
      for ( let field_i = 0; field_i < fields_to_set.length; field_i++ ) {
        ...
        // TODO: Mutate `.there_is_another` here instead? Could then possibly
        // calculate the `.there_is_another` value before calling setVariable(). Would need to pass in fake
        // var_data. Then what data to we want to give to the developer in there reports?
        // Removes the need to pass down `from_table`.
        // TODO: What if `.there_is_another` shows up again after it's already been set to False. Is that possible?
        if ( was_set ) {
