...
module.exports = {
  ...
  afterStep: async function afterStep(scope, options = {waitForShowIf: false, tabToWaitFor: '', navigated: false, waitForTimeout: 0}) {
    ...
    *    TODO: split this into `afterField`/`afterStep`/`afterNavigation` etc. or
    *    possibly split tapping buttons or links into another function than tapping
    *    other things?
    *    TODO: Update to new cucumberjs `AfterStep` for actual after step stuff. */
  ...
  tapElement: async function tapElement(scope, elem, waitForTimeout=0) {
    /** Tap a given element, wait for navigation, and do appropriate related things. */
    // Check if the element exists, though our functions should know to only pass good things.
    ...
    // get text of element for messages

    // get start url

    // wait for click or navigation or invalid user input message

    // loop until new url/navigation/system error/invalid input message/new
    // target var (can't count on that), or timeout

    // wait for 200ms to give elements time to 'show' (is there a faster way?)

    // afterStep

  },  // Ends scope.tapElement()

  tapToNavigate: async function tapToNavigate(scope, elem, tabToWaitFor='', waitForTimeout=0) {
    /** Tap a link or submit button, wait for navigation, and do
    *    appropriate related things.
    */
    ...
    console.log(`navigated? start: ${ start_url }, end: ${ end_url }`);
    // await scope.take_a_screenshot( scope, { path: `${ scope.paths.artifacts }/${Date.now()}.jpg` });
  ...
  continue: async function ( scope ) {
    ...
    await scope.tapToNavigate( scope, elem );
  },  // Ends scope.continue()
  ...
  tapTab: async function ( scope, tab_id ) {
    ...
    await scope.tapToNavigate( scope, elem, tab_id );
    // handle tapping in here since it doesn't call on the server, then
    // afterStep
  },
  ...
  tapElementBySelector: async function ( scope, selector, waitForTimeout ) {
    ...
    expect( elem, error_msg ).to.exist;
    await scope.tapToNavigate( scope, elem, '', waitForTimeout );
  },
  ...
  setLanguage: async function ( scope, { language }) {
    ...
    if ( language ) {
      ...
      await scope.tapToNavigate( scope, lang_link );
    }
  ...
  get_handle_from_field: async function ( scope, { field }) {
    ...
    let handle = await scope.page.evaluateHandle(( selector ) => {
      ...
      if ( elem == null ) {
        throw new Error( document.querySelector( '#daquestion' ).innerHTML );
      }
  ...
  enter_answer: {
    button: async function ( scope, { handle, answer }) { await scope.tapToNavigate( scope, handle ); },
  ...
  steps: {
    ...
    tap_term: async ( scope, phrase ) => {
      ...
      await scope.tapToNavigate( scope, link );
  ...
  get_random_input_for: {
    ...
    date: async function ( scope ) {
      ...
      // This seems to sometimes give a date without a year
    ...
    BirthDate: async function ( scope ) {
      // TODO: Interact with visible date fields to match this
