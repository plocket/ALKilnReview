...
module.exports = {
  ...
  tap_navigation_elem: async function(scope, { elem }) {
    ...
    // TODO: not all pages have ids. Get html from get_nav_attempt_winner
  ...
  get_nav_attempt_winner: async function( scope, { elem }) {
    // @rf8 was passing as of 2024-08-07
    ...
    *  */
    ...
    //   // `/interview?` is the correct as per da dev feedback. See #229
// // TODO: Re-do this to only look for 200s that go to an interview page, as before
...
//   // Do this outside, after the event is done. We can then treat
...
// });
    ...
    let winner = await Promise.race([
      ...
      // We get them back in the same order
      ...
      // To separate:
      // 500s responses
      // 305s that go to non-interview pages
      // 200s and 305s that go to interview pages
      // Check for `validation code` later in the response HTML
    ]);
    ...
    // try {
      ...
      .then(()=>console.log(`> end abort in evaluate`))
      .catch((error)=>console.log(`abort in evaluate:`, error.name));
    // } catch (error) {
