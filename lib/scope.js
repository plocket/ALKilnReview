...
module.exports = {
  ...
  tapElement: async function tapElement(scope, elem) {
    ...
    let parentHTML = await elem.evaluate( el => { return el.parentNode.outerHTML });
    console.log( parentHTML );

    console.log(6);
    await scope.page.screenshot({ path: `./screenshot_logging_${iter}.jpg`, type: 'jpeg', fullPage: true });
    iter++;
    ...
    winner = await Promise.race([
      Promise.all([  // Error loads page, so no need to detect to keep things short
        ...
        elem.evaluate( (el, activate) => { return el[ activate ](); }, scope.activate),  // Unfortunately, this won't catch an invisible element. Hopefully we'd catch it before now...?
        //elem[ scope.activate ](),  // Was working locally, but not on GitHub??? Chromium version?
  ...
  load: async function ( scope, file_name, attempt_num, start_time ) {
    ...
    // try {
      ...
      // console.log(0.0005, ` attempt:`, attempt_num);
      // console.log( `attempt_timeout:`, attempt_timeout );
      ...
      // TODO:
      // 1. Wait full system timeout to load each time.
      // 1. Try 3 times. Override timeout in order to allow this much time.
      // 1. Warn user about this?
      // 1. Calculating attempt timeouts wrong anyway. For <15 sec attempt_timeout
      // and 30s system timeout, attempting 3 times instead of 2
      // 1. When page is not loading at all, this is a module not found? We can't
      // re-try immediately because it could be a big file loading.
      // 1. We do this only with the initial load. Should we do this with any page load
      // because other tests could interfere with this test?

      console.log( 'loading' );
      console.log(Date.now());
      // 1. Page load is failing, causing the failure of the test (also get 404. unrelated?)
      // 2. It's logging into the console after, I'm not sure why, though, if it's erroring
      // 3. It immediately fails
      // 4. With a 30 second timeout, attempt 2 is happening over 30 seconds later. Why is that?
      // 4b. It should be triggering 15 seconds after the first attempt - half the custom timeout time.
      // try {
        // await scope.page.goto(interview_url, { waitUntil: 'domcontentloaded', timeout: attempt_timeout })
      ...
      // let bodyclassname = await scope.page.evaluate(function () {
    ...
    // } catch ( err ) {
    //   console.log( `caught load error:`, Date.now() );
    //   console.log( err );
    ...
    //   // }
    // } finally {
    //   console.log(`page status:`, end_status);
