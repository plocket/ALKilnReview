...
module.exports = {
  ...
  get_nav_attempt_winner: async function( scope, { elem }) {
    ...
    *  */
    ...
    let response_200s_promise = scope.page.waitForResponse(function (response) {

      // if (signal.aborted) {
      //   console.log("\n\n200 daPageLoad Aborted");
      //   return;
      // }

      // let is_interview_url = /http.*\/interview\?.*\.yml/.test(response.request().url());
      let is_interview_url = /http.*\/interview\?.*\.yml/.test(response.url());
      let status = response.status();
      console.log( `> check 200` );
      if ( is_interview_url && response.ok() ) {
        return response;
      } else { return false; }
    ...
    .then(async function ( response ) {
      ...
      let html = scope.page.evaluate(function() {
        return new Promise(function(resolve2, reject2) {
          $(document).on(`daPageLoad`, function(event) {
            resolve2(document.body.outerHTML); // resolves when the event fires
          });
        });
      })
    })
    .then((result) => {console.log(`> page load`); return result && result.includes('<body>'); })
    .catch(function (error) {
      if (error.name === 'AbortError') {
        console.log('0 AbortError: Fetch request aborted');
      } else {
        console.log( `\n\n200 da page load:`, error );
      }
    });
    ...
    let winner = await Promise.race([
      ...
      Promise.all([ validation_code_id, click_promise, response_200s_promise, /*daPageLoad_promise,*/ dom_loaded_promise, /*, validation_code_promise*/ ]),  // #nav6 (should finish before dom_loaded_promise below)
      Promise.all([ ok_id,              click_promise, response_200s_promise, /*daPageLoad_promise,*/ dom_loaded_promise, /*validation code element, we hope*/ ]),  // #nav1, #nav2, #nav3, ~~#nav6 (validation should be here already if we need it)~~
