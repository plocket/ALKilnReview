...
module.exports = {
  ...
  addToReport: function( scope, { type, value }) {
    ...
    * keys should correspond to a list containing the relevant data. */
    ...
    let scenario = scope.makeSureReportPartsExist( scope );
  ...
  makeSureReportPartsExist: function( scope ) {
  ...
  _get_sources_folder: function ( scope ) {
    ...
    if (session_vars.get_sources_folder()) {
      return session_vars.get_sources_folder();
    }
    ...
    try {
      ...
      if (dirs.length < 1) {
        ...
        scope.addToReport( scope, { type: `warning`, value: msg});
        throw Error(msg);  // Most people will want to know the sources folder path;
      }
      for (let dir of dirs) {
        pkg_paths.push(`${docassemble_pkg_path}/${dir}/data/sources`);
      }
    } catch ( error_da ) {
    ...
    if (pkg_paths.length > 1 && pkg_paths.length - err_paths.length > 1) {
      scope.addToReport( scope, { type: `warning`, value: `You have multiple folders in the 'docassemble' folder (${ pkg_paths }). These tests will use the first folder with a "data/sources" subfolder (${chosen_dir}).` });
    }
    return chosen_dir;
  },
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    let chosen_dir = scope._get_sources_folder(scope);
