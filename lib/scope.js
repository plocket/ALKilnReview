  ...
  get_nav_attempt_winner: async function( scope, { elem }) {
    ...
    *  */
    ...
    let response_200s_promise = scope.page.waitForResponse(function (response) {
      ...
      return is_right_url && response.ok();
    ...
    // We need a different check.
    let redirect_id = 305;  // #nav7, sign in redirect
    let response_305_promise = scope.page.waitForResponse(function (response) {
      // THIS CAN'T BE THE INTERVIEW URL, JUST THE SERVER URL
      // let is_right_url = /http.*\/interview\?.*\.yml/.test(response.request().url());
      let is_right_url = /http.*\/interview\?.*\.yml/.test(response.url());
      let status = response.status();
      console.log( `> check 305` );
      return is_right_url && status === 305;
    }, { signal: mission_status }).then(
      /*Listen for daPageLoad. Thank you @willmorningstar!*/
    ...
    // // let daPageLoad_promise = new Promise(async function (resolve, reject) {
    //   let daPageLoad_promise = await scope.page.evaluate(function() {
    ...
    let winner = await Promise.race([
      ...
      Promise.all([ system_error_id,    click_promise, response_500s_promise,   dom_loaded_promise, /*daPageLoad_promise*/ ]),  // #nav4
      Promise.all([ redirect_id,        click_promise, response_305_promise,    dom_loaded_promise, /*daPageLoad_promise*/ ]),  // #nav7
      Promise.all([ validation_code_id, click_promise, response_200s_promise,   dom_loaded_promise, daPageLoad_promise, validation_code_promise ]),  // #nav6 (should finish before dom_loaded_promise below)
      Promise.all([ ok_id,              click_promise, response_200s_promise,   dom_loaded_promise, daPageLoad_promise, /*validation code element, we hope*/ dom_loaded_promise ]),  // #nav1, #nav2, #nav3, ~~#nav6 (validation should be here already if we need it)~~
