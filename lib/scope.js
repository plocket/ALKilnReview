...
module.exports = {
  ...
  setField: {
    button: async function ( scope, { handle, set_to }) { await scope.tapElement( scope, handle ); return true; },
    ...
    select: async function ( scope, { handle, set_to }) {
      ...
      // TODO: Can these be double encoded?
        ...
        let base64_option = await scope.page.$(`option[value="${ base64_name }"]`);
        if (base64_option) {
  ...
  setFields: async function ( scope, {
    ...
    */
    ...
    if ( error.was_found ) {
      ...
      let trigger_map = {};
      for (let field of fields) {
        if (!field.was_set && field.guesses.length > 0) {
          for (let guess of field.guesses) {
            if (guess["var"] in field_map) {
              field_map[guess["var"]].push(guess["value"]);
            } else {
              field_map[guess["var"]] = [guess["value"]];
            }
            trigger_map[guess["var"]] = field.trigger;
          }
        }
      }
      for (let field in field_map) {
        console.log(`didn't set field: ${ JSON.stringify(field) }: ${ JSON.stringify(field_map[field]) } (${ trigger_map[field] })`); 
      }
    }
    return error;
