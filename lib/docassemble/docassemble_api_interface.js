...
da_i.throw_an_error_if_server_is_not_responding = async function ( options ) {
  ...
  */
  ...
  try {
    ...
    let response = await _da_REST.health_check( timeout );
...
da_i.is_server_responding = async function ( dev_id_options ) {
  /** For non-in-playground tests, do a health check to
  *    return if server is up. For in-playground tests, return true.
  *    See https://docassemble.org/docs/api.html#user. */

  // Playground interview will itself timeout. No way to detect that.
  if ( session_vars.get_origin() === 'playgroundreresponse' ) {
    return true;
  }
...
da_i.wait_for_package_to_install = async ( task_id, options ) => {
...
da_i.wait_for_server_to_restart = async ( task_id, options ) => {
  ...
  *     that aren't equivalent to the server being done with a task. */
  ...
  // Use either values handed in or default values
  options = options || {};
  let max_time = options.timeout || DEFAULT_MAX_REQUEST_MS;
  let wait_between_loops = options.wait_between_loops || ( 4 * 1000 );
  let wait_for_response = options.wait_for_response || ( 1 * 1000 );
  let max_loops = options.max_loops || max_time/( wait_between_loops + wait_for_response );
  // Flags for stopping the `while` loop
  let num_tries = 0;
  let done_restarting = false;

  // Why loop instead of one long max_time? Because it might not just hang.
  // We might get a return value that's just `waiting`, meaning the
  // task is not done.
  while ( !done_restarting && num_tries <= max_loops ) {
    num_tries++;
    // Wait only a short time for the task to finish, not the full default max_time
    done_restarting = await da_i.has_task_finished( task_id, { max_time: wait_for_response } );
    if ( !done_restarting ) {
      if ( num_tries < max_loops ) {
        ...
        // When the server is busy, the task max_time causes this loop to actually
        ...
        let server_msg = `ALKiln setup: The docassemble server `
          + `didn't finish uploading the package after ${ max_time/1000 } `
          + `seconds. Some things you can try: check your server is up, try `
          + `re-running the test, and try pulling the package manually `
          + `into the playground. If you want to give your server more `
          + `timem to reload, set the MAX_SECONDS_FOR_SETUP input for this `
          + `action to a higher value than ${ DEFAULT_MAX_REQUEST_MS }.`;
  ...
  *    See https://docassemble.org/docs/api.html#restart_status. */
  ...
  try {
    ...
    if ( response.data.status === 'completed' ) {
      ...
      log.info({ pre: `The docassemble server is still running the task with the id ${ task_id }.` });
      return false;
    ...
    // Timeout means the server is busy.
    if ( is_timeout_error( error ) ) {
      log.info({ pre: `The docassemble server timed out on task ${ task_id }.` });
      return false;
    }

    log.error({ pre: `Tried to check that the task ${ task_id } was done, but ran into an unexpected error.` });
    throw error;
