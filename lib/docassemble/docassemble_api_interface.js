...
da_i.get_interview_urls = async function ( interview_list_options ) {
  /* Get a list of urls for interviews on the server. */

  // Defaults
  let { timeout } = interview_list_options ||  { timeout: DEFAULT_MAX_REQUEST_MS };

  log.info({ pre: `Getting list of interviews on ${ session_vars.get_da_server_url() }` });

  // Construct the list of urls of all interviews available on the server
  try {
    let response = await _da_REST.get_interview_list( timeout );

    let urls = [];
    for ( let interview of response.data.interviews ) {
      urls.push( `${ session_vars.get_da_server_url() }${ interview.link }` );
    }
    return urls;

  } catch ( error ) {
    log.error({ pre: `Could not get list of interviews on ${ session_vars.get_da_server_url() }` });
  }

};  // Ends da_i.get_interview_urls()
