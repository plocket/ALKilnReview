...
// node -e 'require("./lib/docassemble/docassemble_api_interface.js").delete_interview(...)'
da_i.quietly_try_to_delete_interviews = async function ({ interview_vars = null, options = {} }) {
...
da_i.any_of_these_interviews_exist = async function ({ interviews = {}, options = {} }) {
  /** Return true if any of the given interviews exist on the server, otherwise false. */
  // Defaults
  let { timeout } = options || { timeout: DEFAULT_MAX_REQUEST_MS };

  let matching_server_interviews = await da_i.get_interviews({ interviews, options });
  return Object.keys(matching_server_interviews).length > 1;
};  // Ends da_i.any_of_these_interviews_exist()
...
da_i.get_interviews = async function ({ interviews = {}, options = {} }) {
  ...
  if ( interviews ) {
    for ( let interview_vars in interviews ) {
...
da_i.get_interview = async function ({ interview_vars, options = {} }) {
  ...
  if ( interview_vars && interview_vars.uid ) {
    try {
      log.debug({ type: `da api info`, code: `ALK9999`, pre: `Trying to get the interview with the id ${ interview_vars.uid }.` });
      // Actual work:
      let response = await _da_REST.get_interview( timeout, interview_vars );
      log.debug({ type: `da api info`, code: `ALK9999`, pre: `Got ${ response.session }.`, data: response });
      ...
      if ( error.status === 400 ) {  // error reading interview list
        msg = `${ error.status } "${ error.statusText }" Could not find interview ${ interview_vars.uid }?`
      }
      log.debug({ type: `da api info`, code: `ALK9999`, pre: msg, data: error });
    }
  } else {
    log.debug({ type: `da api info`, code: `ALK9999`, pre: `There is no interview to get.` });
  }
